...
class AnnParser:
    ...
    __ann_re = re.compile(
        r"(/\*@" + __any_re + r"*?@\*/)" + r"|" + __pragma_end + r"|" + __pragma_beg
    )
        ...
        for i in range(0, len(code_seq)):
            ...
            if is_ann:
                ...
                if self.__leader_ann_re.match(code):
                    ...
                    if trailer_ipos == -1:
                        err(
                            "orio.main.ann_parser: %s: no matching trailer annotation exists"
                            % code_line_no
                        )

                    # apply recursions on the annotation body and the trailing code sequence
                    body_code_seq = self.__markAnnCodeRegions(
                        code_seq[i + 1 : trailer_ipos]
                    )
                    trailing_code_seq = self.__markAnnCodeRegions(
                        code_seq[trailer_ipos + 1 :]
                    )

                    # return the code sequence with annotation code regions
                    marked_code_seq.append(
                        [code_seq[i], body_code_seq, code_seq[trailer_ipos]]
                    )
                ...
                # if a trailer annotation
                    ...
                    err(
                        "orio.main.ann_parser: %s: no matching leader annotation exists in \n%s"
                        % (code_line_no, code)
                    )
    ...
    def __getCodeSeq(self, code, line_no):
        ...
        while True:
            ...
            if not self.__leader_ann_re.match(ann) and not self.__trailer_ann_re.match(
                ann
            ):
                warn(
                    "orio.main.ann_parser warning:%s: unrecognized form of annotation, skipping..."
                    % ann_line_no
                )
                skip = True
    ...
    def __getModuleInfo(self, code, line_no):
        ...
        mindex = 2
        if match_obj.group(mindex) is None:
            mindex += 3
    ...
    def __convertToCodeFragment(self, code):
        ...
        if isinstance(code, list):
            ...
            if len(code) != 3:
                err(
                    "orio.main.ann_parser internal error:  the code list must have a length of three "
                )
            ...
            mname, mname_line_no, mcode, mcode_line_no = self.__getModuleInfo(
                leader, leader_line_no
            )
            leader_cfrag = orio.main.code_frag.LeaderAnn(
                leader,
                leader_line_no,
                leader_indent_size,
                mname,
            ...
            trailer_cfrag = orio.main.code_frag.TrailerAnn(
                trailer, trailer_line_no, trailer_indent_size
            )

            # create a code-region code-fragment
            return orio.main.code_frag.AnnCodeRegion(
                leader_cfrag, cfrags, trailer_cfrag
            )
        ...
        # a non-annotation
            ...
            if is_ann:
                err(
                    "orio.main.ann_parser internal error: %s: unexpected annotation"
                    % line_no
                )
