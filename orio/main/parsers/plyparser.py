...
def p_error(t):
    ...
    if t:
        ...
        pos = (col - 1) * " "
        err(
            "[orio.module.inline.fparser] %s: unexpected symbol '%s' at line %s, column %s:\n\t%s\n\t%s^"
            % (filename, t.value, t.lexer.lineno, col, line, pos)
        )
    else:
        err(
            "[orio.module.inline.fparser] internal error, please email source code to norris@mcs.anl.gov"
        )
