...
def p_main_program(p):
    ...
    if p[1]:
        lineno = p.linespan(1)[0]
    elif p[2]:
        lineno = p.linespan(2)[0]
    elif p[3]:
        lineno = p.linespan(3)[0]
    elif p[4]:
        lineno = p.linespan(4)[0]
    else:
        lineno = p.linespan(5)[0]
...
def p_expression_2(p):
    ...
    p[0] = ast.BinOpExp(
        p[1], p[3], ast.BinOpExp.COMMA, p.lineno(1) + __start_line_no - 1
    )
...
def p_assignment_expression_2(p):
    ...
    if p[2] == "=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.EQ_ASGN, p.lineno(1) + __start_line_no - 1
        )
    elif p[2] in ("*=", "/=", "%=", "+=", "-="):
        lhs = p[1].replicate()
        rhs = None
        if p[2] == "*=":
            rhs = ast.BinOpExp(
                p[1], p[3], ast.BinOpExp.MUL, p.lineno(1) + __start_line_no - 1
            )
        elif p[2] == "/=":
            rhs = ast.BinOpExp(
                p[1], p[3], ast.BinOpExp.DIV, p.lineno(1) + __start_line_no - 1
            )
        elif p[2] == "%=":
            rhs = ast.BinOpExp(
                p[1], p[3], ast.BinOpExp.MOD, p.lineno(1) + __start_line_no - 1
            )
        elif p[2] == "+=":
            rhs = ast.BinOpExp(
                p[1], p[3], ast.BinOpExp.ADD, p.lineno(1) + __start_line_no - 1
            )
        elif p[2] == "-=":
            rhs = ast.BinOpExp(
                p[1], p[3], ast.BinOpExp.SUB, p.lineno(1) + __start_line_no - 1
            )
        else:
            err(
                "orio.main.parsers.fparser internal error:  missing case for assignment operator"
            )
        p[0] = ast.BinOpExp(
            lhs, rhs, ast.BinOpExp.EQ_ASGN, p.lineno(1) + __start_line_no - 1
        )
    else:
...
def p_logical_and_expression_2(p):
    ...
    p[0] = ast.BinOpExp(
        p[1], p[3], ast.BinOpExp.LAND, p.lineno(1) + __start_line_no - 1
    )
...
def p_equality_expression_2(p):
    ...
    if p[2] == "==":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.EQ, p.lineno(1) + __start_line_no - 1
        )
    elif p[2] == "!=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.NE, p.lineno(1) + __start_line_no - 1
        )
    else:
...
def p_relational_expression_2(p):
    ...
    if p[2] == "<":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.LT, p.lineno(1) + __start_line_no - 1
        )
    elif p[2] == ">":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.GT, p.lineno(1) + __start_line_no - 1
        )
    elif p[2] == "<=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.LE, p.lineno(1) + __start_line_no - 1
        )
    elif p[2] == ">=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.GE, p.lineno(1) + __start_line_no - 1
        )
    else:
...
def p_multiplicative_expression_2(p):
    ...
    if p[2] == "*":
        ...
        err(
            "orio.main.parsers.fparser internal error:  unknown multiplicative operator"
        )
...
def p_error(p):
    err(
        'orio.main.parsers.fparser:%s: grammatical error: "%s"'
        % ((p.lineno + __start_line_no - 1), p.value)
    )
    sys.exit(1)
...
def setup_regen(debug=1, outputdir="."):
    ...
    try:
        os.remove(parsetabfile)
    except:
        pass

    parser = parse.ply.yacc.yacc(
        debug=debug,
        optimize=1,
        tabmodule="parsetab",
        write_tables=1,
