    ...
    class __impl:
        ...
        def __init__(self, cmdline={}):
            ...
            if "dry_run" in cmdline.keys():
                ...
                self.dry_run = (
                    False
                )  # When True, don't execute anything, just print commands
            ...
            if "erase_annot" in cmdline.keys():
                ...
                self.erase_annot = (
                    False
                )  # do we need to remove annotations from the output?
            ...
            if "rename_objects" in cmdline.keys():
                ...
                self.rename_objects = (
                    False
                )  # rename compiler files to match original source name
            if "external_command" in cmdline.keys():
                self.external_command = cmdline["external_command"]
            else:
                self.external_command = (
                    ""
                )  # command line being wrapped (not processed, just passed along)
            if "disable_orio" in cmdline.keys():
                self.disable_orio = cmdline["disable_orio"]
            else:
                self.disable_orio = (
                    False
                )  # True when orio is wrapping something other than compilation, e.g., linking
            if "pre_cmd" in cmdline.keys():
                self.pre_cmd = cmdline["pre_cmd"]
            else:
                self.pre_cmd = (
                    ""
                )  # Command string with which to prefix the execution of the Orio-built code
            ...
            self.debug_level = 5
            if (
                "ORIO_DEBUG" in os.environ.keys()
                and os.environ["ORIO_DEBUG"] == "1"
                or "debug" in cmdline.keys()
            ):
                self.debug = True
...
def err(errmsg="", errcode=1, doexit=False):
    sys.stderr.write(
        Globals().error_pre + "ERROR: " + errmsg + Globals().error_post + "\n"
    )
    Globals().loggers["TuningLog"].error(
