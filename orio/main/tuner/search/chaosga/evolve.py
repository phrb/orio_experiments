...
def newSolution(filename, problem, numEvals, growthRate, popMax, popInit, opt=max):
    ...
    organisms = [
        OrganismData(n, problem.generate(), 0, 0, []) for n in range(1, gensize + 1)
    ]
    if filename:
...
def evolve(
    ...
    while numEvals > 0:
        ...
        nextGen = generation_tournament_elite(
            problem,
            population,
            int(min(numEvals, nextgensize - 1)),
            generationCount,
            nameCount,
            opt,
...
def generation_tournament_elite(
    problem, population, popsize, currentGeneration, start, opt
):
    return [
        mutate(
            problem,
            binaryTournamentPair(currentGeneration, problem, i, population, opt),
        )
        for i in range(start, start + popsize)
    ]
...
def binaryTournamentPair(currentGeneration, problem, n, pop, opt):
    return newChild(
        ...
        n,
        currentGeneration,
        opt(choice(pop), choice(pop), key=lambda x: x.fitresult),
        betterOrganism(choice(pop), choice(pop)),
    )
...
def parent_analysis(f):
    ...
    for line in sc:
        ...
        info(
            "orio.main.tuner.search.chaosga.evolve: %s,%s,%s"
            % (best.name, str(best.fitresult), str(best.parents))
        )
