...
class Doptaov(orio.main.tuner.search.search.Search):
    """
    The search engine that uses a random search approach, enhanced with a local
    search that finds the best neighboring coordinate.

    Below is a list of algorithm-specific arguments used to steer the search
    algorithm.  local_distance            the distance number used in the local
    search to find the best neighboring coordinate located within the specified
    distance
    """

    __REMOVED_VARIABLES = "removed_variables" # default: 2
    ...
    def __init__(self, params):
        ...
        if self.total_runs <= 0:
            err((
                "orio.main.tuner.search.doptanova.Doptaov: %s requires the "
