...
class PerfTestCodeGen(object):
    ...
    def __checkDeclFile(self):
        ...
        try:
            ...
            err(
                'orio.main.tuner.ptest_codegen:  cannot read file: "%s"'
                % self.decl_file
            )
    ...
    def __checkInitFile(self):
        ...
        if not match_obj:
            err(
                (
                    'orio.main.tuner.ptest_codegen: no initialization function (named "%s") can be found in the '
                    + 'initialization file: "%s"'
                )
                % (self.init_func_name, self.init_file)
            )
    ...
    def __checkValidationFile(self):
        ...
        if not match_obj:
            err(
                (
                    'orio.main.tuner.ptest_codegen: no validation function (named "%s") can be found in the '
                    + 'validation file: "%s"'
                )
                % (self.validation_func_name, self.validation_file)
            )
...
"""
    ...
    """
        ...
        if self.power:
            begin_outer_measure_code = (
                "__wattprof_total_tag = power_start_measure(__wattprof_daqh,0);"
            )
            end_outer_measure_code = """
    ...
    """
        ...
        ptest_code = self.ptest_skeleton_code.insertCode(
            global_code,
            prologue_code,
            epilogue_code,
            validation_code,
...
class PerfTestCodeGenCUDA(PerfTestCodeGen):
    ...
    def __init__(
        ...
        super(PerfTestCodeGenCUDA, self).__init__(
            input_params,
            input_decls,
            decl_file,
            init_file,
...
class PerfTestCodeGenOpenCL(PerfTestCodeGen):
    ...
    def __init__(
        ...
        super(PerfTestCodeGenOpenCL, self).__init__(
            input_params,
            input_decls,
            decl_file,
            init_file,
...
class PerfTestCodeGenFortran:
    ...
    def __init__(
        ...
        self.__checkInitFile()
        scode = self.__checkSkeletonCodeFile()
        self.ptest_skeleton_code = skeleton_code.PerfTestSkeletonCodeFortran(
            scode, use_parallel_search
        )
        ...
        for is_static, vt, vname, vdims, rhs in input_decls:
            ...
            if len(vdims) == 0:
                ...
                if is_static:
                    ...
                    dim_code = "(%s)" % ")(".join(":")
                    decls.append(
                        "%s, dimension%s, allocatable :: %s" % (vtype, dim_code, vname)
                    )
    ...
    def generate(self, code_map):
        ...
        # get the performance-testing code
        ptest_code = self.ptest_skeleton_code.insertCode(
            declaration_code, prologue_code, epilogue_code, code_map
        )
