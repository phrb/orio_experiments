...
class TuningInfo:
    ...
    def __init__(
        ...
        search_algo, search_time_limit, search_total_runs, search_resume, search_opts = (
            search_info
        )
        ...
        # build arguments
        self.pre_build_cmd = build_info.get(
            "prebuild_cmd"
        )  # command to run before invoking build_cmd
        self.build_cmd = build_info.get(
            "build_cmd"
        )  # command for compiling the generated code
        self.post_build_cmd = build_info.get(
            "postbuild_cmd"
        )  # command to run after building (before running) code, it is applied to the executable
        ...
        self.batch_cmd = build_info.get(
            "batch_cmd"
        )  # command for requesting a batch job
        self.status_cmd = build_info.get(
            "status_cmd"
        )  # command for checking status of submitted batch
        self.num_procs = build_info.get(
            "num_procs"
        )  # the number of processes used to run the test driver
        self.timer_file = build_info.get(
            "timer_file"
        )  # user-specified implementation of the getClock() function
        ...
        self.timing_array_size = (
            timing_array_size
        )  # default an odd number >= pcount_reps
    ...
    def __repr__(self):
        ...
        s += (
            " performance-test skeleton code file: %s \n"
...
class TuningInfoGen:
    ...
    def __genBuildInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if keyw not in ("arg",):
                err(
                    'orio.main.tspec.tune_info: %s: unexpected statement type: "%s"'
                    % (line_no, keyw)
                )
            ...
            if id_name not in (
                ...
                err(
                    'orio.main.tspec.tune_info: %s: unknown build argument: "%s"'
                    % (id_line_no, id_name)
                )
            ...
            if id_name == PREBUILDCMD:
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: post-build command in build section must be a string"
                        % rhs_line_no
                    )
            ...
            if id_name == BUILDCMD:
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: build command in build section must be a string"
                        % rhs_line_no
                    )
            ...
            # Need C compiler for timing routine (even when generating Fortran)
                ...
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: C compiler in build section (CC variable) must be a string"
                        % rhs_line_no
                    )
                ...
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: Fortran compiler in build section (FC variable) must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate the libs
                ...
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: link-time libraries in build section must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate the batch command
                ...
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: batch command in build section must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate the status command
                ...
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: status command in build section must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate the number of processors
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info:%s: number of processors in build section must be a positive integer"
                        % rhs_line_no
                    )
            ...
            # User-specified timer file
                ...
                if not isinstance(rhs, str) or not os.path.exists(rhs):
                    err(
                        "orio.main.tspec.tune_info: %s: specified timer file (%s) not found"
                        % rhs
                    )
    ...
    def __genPerfCounterInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if id_name not in (METHOD, REPS, RANDOM_SEED, TIMING_ARRAY_SIZE):
                err(
                    'orio.main.tspec.tune_info: %s: unknown performance counter argument: "%s"'
                    % (id_line_no, id_name)
                )
            ...
            if id_name == METHOD:
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: performance counting method must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate performance counting repetitions
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info: %s: performance counting repetitions must be a positive integer"
                        % rhs_line_no
                    )
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info: %s: performance counting array size must be a positive integer"
                        % rhs_line_no
                    )
            ...
            # user-specified random seed (otherwhise non-repeatable value based on time is used)
                ...
                if not isinstance(rhs, int):
                    warn(
                        "orio.main.tspec.tune_info: %s: performance counting random seed must be an integer"
                        % rhs_line_no
                    )
                random_seed = rhs
    ...
    def __genPowerInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if id_name not in (METHOD, REPS, RANDOM_SEED, POWER_ARRAY_SIZE):
                err(
                    'orio.main.tspec.tune_info: %s: unknown power measurement argument: "%s"'
                    % (id_line_no, id_name)
                )
            ...
            if id_name == METHOD:
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: power measurement method must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate performance counting repetitions
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info: %s: power measurement repetitions must be a positive integer"
                        % rhs_line_no
                    )
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info: %s: power measurement array size must be a positive integer"
                        % rhs_line_no
                    )
            ...
            # user-specified random seed (otherwhise non-repeatable value based on time is used)
                ...
                if not isinstance(rhs, int):
                    warn(
                        "orio.main.tspec.tune_info: %s: power measurement random seed must be an integer"
                        % rhs_line_no
                    )
                random_seed = rhs
    ...
    def __genSearchInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if id_name not in (ALGO, TLIMIT, TRUNS, RESUME):
                if search_algo == None or not id_name.startswith(
                    search_algo.lower() + "_"
                ):
                    err(
                        'orio.main.tspec.tune_info: %s: unknown search argument: "%s"'
                        % (id_line_no, id_name)
                    )
            ...
            if id_name == ALGO:
                if not isinstance(rhs, str):
                    err(
                        "orio.main.tspec.tune_info: %s: search algorithm must be a string"
                        % rhs_line_no
                    )
            ...
            # evaluate search time limit
                ...
                if (
                    ...
                    err(
                        "orio.main.tspec.tune_info: %s: search time limit must be a positive number"
                        % rhs_line_no
                    )
            ...
            # evaluate the total number of search runs
                ...
                if not isinstance(rhs, int) or rhs <= 0:
                    warn(
                        "orio.main.tspec.tune_info: %s: total number of search runs must be a positive number"
                        % rhs_line_no
                    )
            ...
            # evaluate all other algorithm-specific arguments
                ...
                if id_name == "":
                    warn(
                        'orio.main.tspec.tune_info: %s: invalid algorithm-specific argument name: "%s"'
            ...
            # Resume search if possible starting with the last coordinate
                ...
                if not isinstance(rhs, bool):
                    warn(
                        "orio.main.tspec.tune_info: %s: search resume option must be True or False"
                        % rhs_line_no
                    )
                    search_resume = False
                else:
    ...
    def __genInputVarsInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if keyw == "arg":
                ...
                if id_name == DECL_FILE:
                    if not isinstance(rhs, str):
                        err(
                            "orio.main.tspec.tune_info: %s: declaration file must be a string"
                            % rhs_line_no
                        )

                    if not os.path.exists(rhs):
                        err(
                            'orio.main.tspec.tune_info: %s: cannot find the declaration file: "%s"'
                            % (rhs_line_no, rhs)
                        )
                ...
                # initialization code
                    ...
                    if not isinstance(rhs, str):
                        err(
                            "orio.main.tspec.tune_info: %s: initialization file must be a string"
                            % rhs_line_no
                        )

                    if not os.path.exists(rhs):
                        warn(
                            'orio.main.tspec.tune_info: %s: cannot find the initialization file: "%s"'
                            % (rhs_line_no, rhs)
                        )
                ...
                # unknown argument name
                    ...
                    err(
                        'orio.main.tspec.tune_info: %s: unknown input variable argument: "%s"'
                        % (id_line_no, id_name)
                    )

            # evaluate declarations
            elif keyw == "decl":
                _, _, (id_name, id_line_no), type_seq, dim_exp_seq, (
                    rhs,
                    rhs_line_no,
                ) = stmt
                ...
                if is_static and is_dynamic:
                    err(
                        "orio.main.tspec.tune_info: %s: a declared variable cannot be both static and dynamic"
                        % line_no
                    )

                if (not is_array) and (is_static or is_dynamic):
                    err(
                        "orio.main.tspec.tune_info: %s: static and dynamic types are only for arrays"
                        % line_no
                    )

                if is_array and (not is_static) and (not is_dynamic):
                    err(
                        "orio.main.tspec.tune_info: %s: missing static/dynamic type for arrays variable"
                        % line_no
                    )
                ...
                if len(type_seq) > 2:
                    err(
                        'orio.main.tspec.tune_info: %s: unrecognized type name: "%s"'
                        % (line_no, type_seq[0])
                    )
        ...
        if len(ivar_decls) > 0:
            ...
            if ivar_decl_file:
                warn(
                    (
                        "orio.main.tspec.tune_info:  since input variables are declared in the tuning specification, "
                        + 'the declaration file "%s" is not needed'
                    )
                    % ivar_decl_file
                )
            ...
            if ivar_init_file == None:
                for _, _, iname, _, rhs in ivar_decls:
                    if rhs == None:
                        warn(
                            (
                                "orio.main.tspec.tune_info:  missing an initial value in the input variable "
                                + 'declaration of "%s" in the tuning specification'
                            )
                            % iname
                        )
            ...
            # declarations are generated by the driver; initializations are provided by the user
                ...
                for _, _, iname, _, rhs in ivar_decls:
                    if rhs != None:
                        warn(
                            (
                                "orio.main.tspec.tune_info: since initializations of the input variables are provided "
                                + 'by the user in file "%s", input variable "%s" must be '
                                + "declared without an initial value"
                            )
                            % (ivar_init_file, iname)
                        )
            ...
            if ivar_decl_file == None or ivar_init_file == None:
                warn(
                    "orio.main.tspec.tune_info:  missing declarations and/or initializations of the input "
        ...
        for stmt in stmt_seq:
            ...
            if id_name == SKELETON_CODE_FILE:
                ...
                if not os.path.exists(rhs):
                    warn(
                        (
                            "orio.main.tspec.tune_info: %s: cannot find the file specified in performance-test "
                            + 'skeleton code file: "%s"'
                        )
                        % (rhs_line_no, rhs)
                    )
        ...
        for stmt in stmt_seq:
            ...
            if keyw == "arg":
                ...
                if id_name == VALIDATION_FILE:
                    if not isinstance(rhs, str):
                        err(
                            "orio.main.tspec.tune_info: %s: validation file must be a string"
                            % rhs_line_no
                        )

                    if not os.path.exists(rhs):
                        err(
                            'orio.main.tspec.tune_info: %s: cannot find the validation file: "%s"'
                            % (rhs_line_no, rhs)
                        )
                ...
                # unknown argument name
                    ...
                    err(
                        'orio.main.tspec.tune_info: %s: unknown validation argument: "%s"'
                        % (id_line_no, id_name)
                    )
    ...
    def __genOtherInfo(self, stmt_seq, def_line_no):
        ...
        for stmt in stmt_seq:
            ...
            if keyw not in ("arg"):
                err(
                    'orio.main.tspec.tune_info: %s: unexpected statement type in [Other] section: "%s"'
                    % (line_no, keyw)
                )
            ...
            if keyw == "arg":
                ...
                if id_name == DEVICE:
                    if not isinstance(rhs, str):
                        err(
                            "orio.main.tspec.tune_info: %s: device specification must be a path (string)"
                            % rhs_line_no
                        )

                    if not os.path.exists(rhs):
                        err(
                            'orio.main.tspec.tune_info: %s: cannot find the device specification file: "%s"'
                            % (rhs_line_no, rhs)
                        )
                ...
                # unknown argument name
                    ...
                    err(
                        'orio.main.tspec.tune_info: %s: unknown argument in [Other] section: "%s"'
                        % (id_line_no, id_name)
                    )
    ...
    def generate(self, stmt_seq):
        ...
        for stmt in stmt_seq:
            ...
            if dname not in (
                ...
                err(
                    'orio.main.tspec.tune_info: %s: unknown definition name: "%s"'
                    % (dname_line_no, dname)
                )
            ...
            if dname == BUILD:
                (
                    ...
                    fc,
                    timer_file,
                ) = self.__genBuildInfo(body_stmt_seq, line_no)
                if build_cmd == None:
                    err(
                        "orio.main.tspec.tune_info: %s: missing build command in the build section"
                        % line_no
                    )
                ...
                if (batch_cmd != None and status_cmd == None) or (
                    ...
                    warn(
                        (
                            "orio.main.tspec.tune_info: %s: both batch and status commands in build section "
                            + "must not be empty"
                        )
                        % line_no
                    )
                ...
                if batch_cmd == None and num_procs > 1:
                    warn(
                        (
                            "orio.main.tspec.tune_info: %s: number of processors in build section must be greater than "
                            + "one for non-batch (or non-parallel) search"
                        )
                        % line_no
                    )
            ...
            # performance counter definition
                ...
                pcount_method, pcount_reps, random_seed, timing_array_size = self.__genPerfCounterInfo(
                    body_stmt_seq, line_no
                )
            ...
            # Power/energy measurement
                ...
                power_method, power_reps, random_seed, power_array_size = self.__genPowerInfo(
                    body_stmt_seq, line_no
                )
            ...
            # search definition
                ...
                (
                    ...
                    search_resume,
                    search_opts,
                ) = self.__genSearchInfo(body_stmt_seq, line_no)
                default_s_algo, default_s_tlimit, default_s_truns, default_s_resume, _ = (
                    search_info
                )
                if search_algo == None:
            ...
            # performance parameters definition
                ...
                pparam_params, pparam_constraints = self.__genPerfParamsInfo(
                    body_stmt_seq, line_no
                )
                if len(pparam_constraints) > 0 and len(pparam_params) == 0:
                    err(
                        "orio.main.tspec.tune_info: %s: constraints require parameters definitions"
                        % dname_line_no
                    )
                ...
                cmdline_params, cmdline_constraints = self.__genPerfParamsInfo(
                    body_stmt_seq, line_no
                )
                if len(cmdline_constraints) > 0 and len(cmdline_params) == 0:
                    err(
                        "orio.main.tspec.tune_info: %s: command line constraints require command line parameters definitions"
                        % dname_line_no
                    )
            ...
            # input parameters definition
                ...
                iparam_params, iparam_constraints = self.__genInputParamsInfo(
                    body_stmt_seq, line_no
                )
            ...
            # input variables definition
                ...
                (ivar_decls, ivar_decl_file, ivar_init_file) = self.__genInputVarsInfo(
                    body_stmt_seq, line_no
                )
            ...
            # performance-test code definition
                ...
                (ptest_skeleton_code_file,) = self.__genPerfTestCodeInfo(
                    body_stmt_seq, line_no
                )
        ...
        if ivar_info == None:
            err(
                "orio.main.tspec.tune_info:  missing input variables definition in the tuning specification"
            )
