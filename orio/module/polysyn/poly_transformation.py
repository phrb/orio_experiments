...
class PolyTransformation:
    ...
    def __plutoTransform(self, code):
        ...
        if os.popen("polycc").read() == "":
            err(
                'orio.module.polysyn.poly_transformation:  Pluto is not installed. Cannot use "polycc" command.'
            )
        ...
        if use_tiling:
            ...
            try:
                ...
                err(
                    "orio.module.polysyn.poly_transformation:  cannot write to file: %s"
                    % ts_fname
                )
        ...
        try:
            ...
            err(
                "orio.module.polysyn.poly_transformation:  cannot open file for writing: %s"
                % fname
            )
        ...
        try:
            ...
            err(
                "orio.module.polysyn.poly_transformation:  failed to run command: %s"
                % cmd
            )
        ...
        for f in removed_fnames:
            try:
                ...
                err(
                    "orio.module.polysyn.poly_transformation:  failed to remove file: %s"
                    % f
                )
        ...
        if not os.path.exists(plutogen_fname):
            err(
                "orio.module.polysyn.poly_transformation:  failed to generate Pluto-transformed code"
            )
        try:
