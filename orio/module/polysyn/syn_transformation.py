...
class SynTransformation:
    ...
    def __init__(
        self, verbose, permut, unroll_factors, scalar_replace, vectorize, rect_regtile
    ):
    ...
    def __executeAnnTool(self, code):
        ...
        try:
            ...
            err(
                "orio.module.polysyn.syn_transformation:  cannot write to file: %s"
                % ifname
            )
        ...
        try:
            ...
            err(
                'orio.module.polysyn.syn_transformation:  failed to execute command: "%s"'
                % cmd
            )
        ...
        try:
            ...
            err(
                "orio.module.polysyn.syn_transformation:  cannot read file: %s" % ofname
            )
        ...
        for fname in [ifname, ofname]:
            try:
                ...
                err(
                    "orio.module.polysyn.syn_transformation:  cannot delete files: %s"
                    % fname
                )
    ...
    def __generateAnnotations(self, code):
        ...
        while True:
            ...
            if not tail_m:
                err(
                    "orio.module.polysyn.syn_transformation: hotspot tail tag cannot be found"
                )
            ...
            pparams = []
            if permut_code:
                pparams.append(permut_code)
            if regtile_code:
                pparams.append(regtile_code)
            if srep_code:
                pparams.append(srep_code)
            if vec_code:
                pparams.append(vec_code)
