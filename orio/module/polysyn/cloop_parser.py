...
class CLoopParser:
    ...
    def __parseForLoop(self, linestr, line_num):
        ...
        if pos < 0:
            err(
                "orio.module.polysyn.cloop_parser internal error: for-loop is not declared in one line"
            )
        m2 = re.match(r"\s*\{\s*$", linestr[m1.end() :][pos + 1 :])
        if not m2:
            err(
                "orio.module.polysyn.cloop_parser internal error: for-loop does not have an opening curly brace"
            )
        m3 = re.match(r"^\s*(\w+)\s*=", linestr[m1.end() :][:pos])
        if not m3:
            err(
                "orio.module.polysyn.cloop_parser internal error: init-expression of for loop cannot be empty"
            )
        loop_id = m3.group(1)
    ...
    def __parseIfStmt(self, linestr, line_num):
        ...
        if pos < 0:
            err(
                "orio.module.polysyn.cloop_parser internal error: if-statement is not declared in one line"
            )
        m2 = re.match(r"\s*\{\s*$", linestr[m1.end() :][pos + 1 :])
        if not m2:
            err(
                "orio.module.polysyn.cloop_parser internal error: if-statement does not have an opening curly brace"
            )
    ...
    def __parseMacroStmt(self, linestr, line_num):
        ...
        if pos < 0:
            err(
                "orio.module.polysyn.cloop_parser internal error: Sn(...)-statement is not declared in one line"
            )
        m2 = re.match(r"\s*;\s*$", linestr[m1.end() :][pos + 1 :])
        if not m2:
            err(
                "orio.module.polysyn.cloop_parser internal error: Sn(...)-statement does not end with a semicolon"
            )
    ...
    def __parseCodeLine(self, linestr, line_num):
        ...
        return (
            self.__parseForLoop(linestr, line_num)
            or self.__parseIfStmt(linestr, line_num)
            or self.__parseClosingBrace(linestr, line_num)
            or self.__parseMacroStmt(linestr, line_num)
            or self.__parseDeclarations(linestr, line_num)
            or self.__parseDirective(linestr, line_num)
            or self.__parseAssignStmt(linestr, line_num)
            or self.__parseElseStmt(linestr, line_num)
            or self.__parseWhitespace(linestr, line_num)
        )
    ...
    def __moveOpeningTag(self, pluto_code):
        ...
        if (not open_m) or (not close_m):
            err(
                "orio.module.polysyn.cloop_parser internal error: cannot find the opening and closing tags for "
                + "the Cloog code"
            )
        ...
        if len(rests) == 0:
            err(
                "module.polysyn.cloop_parser: internal-error:polysyn: Cloog code cannot be empty",
                doexit=True,
            )
        ...
        while True:
            ...
            if pcl[0] == "forloop":
                ...
                while True:
                    ...
                    if i == len(parsed_clines):
                        err(
                            "orio.module.polysyn.cloop_parser internal error: cannot find a matching closing brace"
                        )
                    pcl = parsed_clines[i]
        ...
        for cl in clines:
            ...
            if isinstance(cl, list):
                ...
                if cl[0][0] == "forloop":
                    ...
                    if inests:
                        inests = [
                            ([(loop_id, head_l, tail_l, body)] + n) for n in inests
                        ]
                    else:
                    ...
                    err(
                        'orio.module.polysyn.cloop_parser internal error: unrecognized scope: "%s"'
                        % cl
                    )
    ...
    def __findHotspotInnerLoopNest(self, clines):
        ...
        if len(hotspot_inests) > 1:
            err(
                "orio.module.polysyn.cloop_parser internal error: hotspot must not contain two (or more) "
    ...
    def __isParallelLoop(self, line_num, clines):
            ...
            if isinstance(cl, list):
                ...
                if cl[0][0] == "forloop":
                    ...
                    if line_num == head_l:
                        return (
                            prev_cl
                            and not isinstance(prev_cl, list)
                            and prev_cl[1].startswith("#pragma omp parallel")
                        )
        ...
        err(
            'orio.module.polysyn.cloop_parser internal error: code at line number "%s" cannot be found'
            % line_num
        )
            ...
            if isinstance(cl, list):
                ...
                if cl[0][0] == "forloop":
                    ...
                    n_outer_loop_nest = outer_loop_nest[:] + [
                        (loop_id, head_l, tail_l, body)
                    ]
                    if line_num == head_l or line_num == tail_l:
                        return n_outer_loop_nest
                    elif head_l < line_num < tail_l:
                        return self.__findOuterLoopNest(
                            line_num, body, n_outer_loop_nest
                        )
                    ...
                    if line_num == head_l or line_num == tail_l:
                        ...
                        if line_num == middle_l:
                            err(
                                "orio.module.polysyn.cloop_parser internal error: hotspot cannot be at else keyword"
                            )
                        if head_l < line_num < middle_l:
                            return self.__findOuterLoopNest(
                                line_num, true_stmt, n_outer_loop_nest
                            )
                        elif middle_l < line_num < tail_l:
                            return self.__findOuterLoopNest(
                                line_num, false_stmt, n_outer_loop_nest
                            )
                    elif head_l < line_num < tail_l:
                        return self.__findOuterLoopNest(
                            line_num, true_stmt, n_outer_loop_nest
    ...
    def getHotspotLoopNests(self, pluto_code, hotspots_info):
        ...
        cloog_code, hotspots_info, pluto_code = self.__focusToCloogCode(
        ...
        if len(hotspots_info) == 0:
            err(
                "orio.module.polysyn.cloop_parser: polysyn: cannot find any hotspots in the Cloog code"
            )
        ...
        for i, cl in enumerate(cloog_code.split("\n")):
            ...
            if not d:
                err(
                    'orio.module.polysyn.cloop_parser internal error: unrecognized line of code: "%s"'
                    % cl
                )
            clines.append(d)
        ...
        hotspot_lnests = hotspot_lnests[
            : min(len(hotspot_lnests), self.max_num_hotspots)
        ]
        ...
        if len(hotspot_lnests) == 0:
            err(
                "orio.module.polysyn.cloop_parser: polysyn: hotspot has no loops to be optimized"
            )
