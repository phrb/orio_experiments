...
class CodeGen:
    ...
    def __generateInodeCode(self, parallelize=False):
        ...
        if parallelize:
            ...
            iarrs = [iarr] + [
                "%s%sc" % (ainfo.in_matrix, i)
                for i in range(1, ainfo.out_unroll_factor)
            ]
            iarr_inits = [
                "%s=%s+clength" % (a, iarrs[i]) for i, a in enumerate(iarrs[1:])
            ]
            ovec = "%sc" % ainfo.out_vector
            ovecs = [
                "%s%sc" % (ainfo.out_vector, i)
                for i in range(0, ainfo.out_unroll_factor)
            ]
            ovec_inits = ["%s=%s" % (v, ainfo.init_val) for v in ovecs]
            ovec_stores = [
                "%sc[%s]=%s" % (ainfo.out_vector, i, v) for i, v in enumerate(ovecs)
            ]
            ...
            code += (
                "#pragma omp parallel for shared(%s,%s,%s,%s,%s,%s,%s) private(n)\n"
                % (
                    ainfo.out_vector,
                    ainfo.in_vector,
                    ainfo.in_matrix,
            ...
            code += (
                "  register int clength=%s[start_row+1]-first_col;\n" % ainfo.row_inds
            )
            ...
            ovecs = [
                "%s%s" % (ainfo.out_vector, i)
                for i in range(0, ainfo.out_unroll_factor)
            ]
            ovec_inits = ["%s=%s" % (v, ainfo.init_val) for v in ovecs]
            ovec_stores = [
                "%s[%s]=%s" % (ainfo.out_vector, i, v) for i, v in enumerate(ovecs)
            ]
            col_inds = ainfo.col_inds
        ...
        ivecs = [
            "%s%s" % (ainfo.in_vector, i) for i in range(0, ainfo.in_unroll_factor)
        ]
        ivec_refs = [
            "%s[%s[%s]]" % (ainfo.in_vector, col_inds, i)
        ...
        if len(iarr_inits) > 0:
            code += "    %s %s;\n" % (
                ainfo.data_type,
                ",".join(map(lambda x: "*" + x, iarr_inits)),
            )
        ...
        for ov, ia in zip(ovecs, iarrs):
            ...
            code += ";\n"
        code += "      %s;\n" % "; ".join(
            ["%s+=%s" % (a, ainfo.in_unroll_factor) for a in iarrs]
        )
    ...
    def __generateGccSimdInodeCode(self, parallelize=False):
        ...
        vovec_inits = [
            "%s={%s,%s}" % (r, ainfo.init_val, ainfo.init_val) for r in vovecs
        ]
        vtovecs = ["%st" % v for v in ovecs]
        vtovec_decls = [
            "*%s=(%s *)&%s" % (v, ainfo.data_type, r) for v, r in zip(vtovecs, vovecs)
        ]
        vovec_stores = ["%s=%s[0]+%s[1]" % (r, t, t) for r, t in zip(ovecs, vtovecs)]
        vivecs = [
            "%s%sv" % (ainfo.in_vector, i) for i in range(0, ainfo.in_unroll_factor / 2)
        ]
        ...
        code += "    while (j<=clength-%s) {\n" % ainfo.in_unroll_factor
        code += "      v2df %s;\n" % ",".join(
            "%s={%s,%s}" % (vivecs[i], ivec_refs[2 * i], ivec_refs[2 * i + 1])
        ...
        for vov, via in zip(vovecs, viarrs):
            ...
            code += ";\n"
        code += "      %s;\n" % "; ".join(
            ["%s+=%s" % (a, ainfo.in_unroll_factor / 2) for a in viarrs]
        )
        code += "      %s+=%s;\n" % (col_inds, ainfo.in_unroll_factor)
        code += "      j+=%s;\n" % ainfo.in_unroll_factor
        code += "    }\n"
    ...
    def __generateSseSimdInodeCode(self, parallelize=False):
        ...
        if parallelize:
            ...
            code += "omp_set_num_threads(%s);\n" % ainfo.num_threads
            code += (
                "#pragma omp parallel for shared(%s,%s,%s,%s,%s,%s,%s) private(n,tbuf)\n"
                % (
                    ainfo.out_vector,
                    ainfo.in_vector,
                    ainfo.in_matrix,
        ...
        vovec_assgs = [
            "_mm_storer_pd(&tbuf[%s],%s)" % (2 * i, r) for i, r in enumerate(vovecs)
        ]
        vovec_stores = [
            "%s=tbuf[%s]+tbuf[%s]" % (v, 2 * i, 2 * i + 1) for i, v in enumerate(ovecs)
        ]
        viarrs = [
            ["%sv%s" % (a, i) for i in range(ainfo.in_unroll_factor / 2)] for a in iarrs
        ]
        viarr_inits = [
            ["%s=_mm_load_pd(&%s[%s])" % (r, a, 2 * i) for i, r in enumerate(rs)]
        ...
        code += "      __m128d %s;\n" % ",".join(
            [
                "%s=_mm_setr_pd(%s,%s)"
                % (vivecs[i], ivec_refs[2 * i], ivec_refs[2 * i + 1])
                for i in range(0, ainfo.in_unroll_factor / 2)
            ]
        )
    ...
    def __generateXlcSimdInodeCode(self, parallelize=False):
        ...
        if parallelize:
            ...
            code += "omp_set_num_threads(%s);\n" % ainfo.num_threads
            code += (
                "#pragma omp parallel for shared(%s,%s,%s,%s,%s,%s,%s,zerobuf) private(n,tbuf,xbuf)\n"
                % (
                    ainfo.out_vector,
                    ainfo.in_vector,
                    ainfo.in_matrix,
        ...
        vovec_assgs = [
            "__stfpd(&tbuf[%s],%s)" % (2 * i, r) for i, r in enumerate(vovecs)
        ]
        vovec_stores = [
            "%s=tbuf[%s]+tbuf[%s]" % (v, 2 * i, 2 * i + 1) for i, v in enumerate(ovecs)
        ]
        ...
        viarr_inits = [
            ["%s=__lfpd(&%s[%s])" % (r, a, 2 * i) for i, r in enumerate(rs)]
            for rs, a in zip(viarrs, iarrs)
        ]
        vivecs = [
            "%s%sv" % (ainfo.in_vector, i) for i in range(0, ainfo.in_unroll_factor / 2)
        ...
        code += "    while (j<=clength-%s) {\n" % ainfo.in_unroll_factor
        code += "      %s;\n" % "; ".join(
            "xbuf[%s]=%s" % (i, ivec_refs[i]) for i in range(ainfo.in_unroll_factor)
        )
        code += "      double _Complex %s;\n" % ",".join(
            [
                "%s=__lfpd(&xbuf[%s])" % (vivecs[i], 2 * i)
                for i in range(0, ainfo.in_unroll_factor / 2)
            ]
        )
        for ins in viarr_inits:
    ...
    def __generateCode(self, parallelize=False):
        ...
        if parallelize:
            ...
            code += "omp_set_num_threads(%s);\n" % ainfo.num_threads
            code += (
                "#pragma omp parallel for shared(%s,%s,%s,%s,%s,%s,lbound) private(i)\n"
                % (
                    ainfo.out_vector,
                    ainfo.in_vector,
                    ainfo.in_matrix,
        ...
        code += "  register int %s;\n" % ",".join(
            [
                "lb%s=%s[i%s]" % (i, ainfo.row_inds, "+%s" % i if i else "")
    ...
    def __generateXlcSimdCode(self, parallelize=False):
        ...
        if parallelize:
            ...
            code += "omp_set_num_threads(%s);\n" % ainfo.num_threads
            code += (
                "#pragma omp parallel for shared(%s,%s,%s,%s,%s,%s,lbound,zerobuf) private(i,tbuf,xbuf)\n"
                % (
                    ainfo.out_vector,
                    ainfo.in_vector,
                    ainfo.in_matrix,
        ...
        for i in range(0, ainfo.out_unroll_factor):
            ...
            code += "      %s;\n" % "; ".join(
                "xbuf[%s]=%s[%s[j%s]]"
                % (i, ainfo.in_vector, ainfo.col_inds, "+%s" % i if i else "")
                for i in range(ainfo.in_unroll_factor)
            )
            for j in range(0, ainfo.in_unroll_factor / 2):
                if not j:
                    code += "      double _Complex "
                if j:
                    code += ","
