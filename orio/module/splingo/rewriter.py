...
class Rewriter(object):
    ...
    def transform(self, n):
        ...
        def isAssign(n):
            return (
                isinstance(n, ExpStmt)
                and isinstance(n.exp, BinOpExp)
                and n.exp.oper == BinOpExp.EQ
            )
        ...
        def expandCons(n):
            if isAssign(n):
                            ...
                            getForStmt(
                                ...
                                CompStmt(
                                    [
                                        s2t("stmt", "tmp1 = 0.0"),
                                        getForStmt(
                                            itrs[0], "0", dims[0], itrs[0] + "++", n1
                                        ),
                                        s2t(
                                            "stmt",
                                            repr(self.applyOnce["cacheLhs"])
                                            + " = tmp1",
                                        ),
                                    ]
                                ),
                ...
                lengths = map(
                    lambda x: ParamDec(IdentExp("int"), IdentExp(x)),
