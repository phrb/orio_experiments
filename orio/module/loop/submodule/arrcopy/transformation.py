...
class Transformation:
    ...
    def transform(self):
        ...
        if is_done:
            if decl == None:
                err(
                    "orio.module.loop.submodule.arrcopy.transformation:  array copy optimization was unsuccessful"
                )
            else:
            ...
            err(
                "orio.module.loop.submodule.arrcopy.transformation:  array copy optimization must be applied on a tiled perfect loop nest"
            )
    ...
    def __containARef(self, tnode, is_output):
        ...
        if isinstance(tnode, orio.module.loop.ast.NumLitExp):
            ...
            return (
                self.__containARef(tnode.test, is_output)
            ...
            err(
                "orio.module.loop.submodule.arrcopy.transformation internal error: unprocessed transform statement"
            )
            ...
            err(
                'orio.module.loop.submodule.arrcopy.transformation internal error: unexpected AST type: "%s"'
    ...
    def __replaceARef(self, tnode, aref_str, replacement):
        ...
        if isinstance(tnode, orio.module.loop.ast.NumLitExp):
            ...
            tnode.args = [
                self.__replaceARef(a, aref_str, replacement) for a in tnode.args
            ]
            return tnode
            ...
            tnode.stmts = [
                self.__replaceARef(s, aref_str, replacement) for s in tnode.stmts
            ]
            return tnode
            ...
            tnode.false_stmt = self.__replaceARef(
                tnode.false_stmt, aref_str, replacement
            )
            return tnode
    ...
    def __createArrCopy(self, aref, arr_name, loop_headers, buf_dsizes):
        ...
        for index_id, lbound_exp, ubound_exp, stride_exp in loop_headers:
            sub_exp = orio.module.loop.ast.BinOpExp(
                index_id.replicate(),
            ...
            sub_exp = orio.module.loop.ast.BinOpExp(
                sub_exp, stride_exp.replicate(), orio.module.loop.ast.BinOpExp.DIV
            )
        ...
        copy_loop = orio.module.loop.ast.BinOpExp(
        ...
        for index_id, lbound_exp, ubound_exp, stride_exp in rev_loop_headers:
            copy_loop = self.flib.createForLoop(
                index_id, lbound_exp, ubound_exp, stride_exp, copy_loop
            )
        copy_loop = orio.module.loop.ast.Container(copy_loop)

        # generate the storing loop: <for-loops> X_orig[...][...] = X_buffer[(i-LBi)/STi][...];
        store_loop = orio.module.loop.ast.BinOpExp(
            aref.replicate(),
            intmd_aref.replicate(),
            orio.module.loop.ast.BinOpExp.EQ_ASGN,
        )
        ...
        for index_id, lbound_exp, ubound_exp, stride_exp in rev_loop_headers:
            store_loop = self.flib.createForLoop(
                index_id, lbound_exp, ubound_exp, stride_exp, store_loop
            )
    ...
    def __optimizeCopy(self, stmt, aref_info, outer_lids):
        ...
        if isinstance(stmt, orio.module.loop.ast.ExpStmt):
            ...
            nrinfo = None
            for s in stmt.stmts:
                cstmt, is_done, decl, rinfo = self.__optimizeCopy(
                    s, aref_info, outer_lids
                )
                nstmts.append(cstmt)
                if is_done:
                    if decl != None:
                        if ndecl != None:
                            err(
                                "orio.module.loop.submodule.arrcopy.transformation: array copy optimization cannot work for imperfect "
                                + "loop nests"
                            )
                        else:
                            ndecl = decl
                    ...
                    if nrinfo != None:
                        err(
                            "orio.module.loop.submodule.arrcopy.transformation:  array copy optimization cannot work for imperfect loop nests"
                        )
                    else:
                        nrinfo = rinfo
            stmt.stmts = nstmts
            ...
            (stmt1, is_done1, decl1, rinfo1) = self.__optimizeCopy(
                stmt.true_stmt, aref_info, outer_lids
            )
            (stmt2, is_done2, decl2, rinfo2) = self.__optimizeCopy(
                stmt.false_stmt, aref_info, outer_lids
            )
            ...
            if sets.Set(ivar_names) <= sets.Set(outer_lids):
                ...
                ind_inames = filter(
                    lambda i: (
                        self.clib.containIdentName(lbound_exp, i)
                        or self.clib.containIdentName(ubound_exp, i)
                        or self.clib.containIdentName(stride_exp, i)
                    ),
                    outer_lids,
                )
            ...
            # otherwise
                ...
                (nstmt, is_done, decl, rinfo) = self.__optimizeCopy(
                    stmt.stmt, aref_info, outer_lids
                )
                ...
                if index_id.name in ind_inames:
                    (decl, intmd_aref, copy_loop, store_loop) = self.__createArrCopy(
                        aref, arr_name, loop_headers, buf_dsizes
                    )
                    prologue = [copy_loop]
                ...
                if index_id.name in rem_inames:
                    ...
                    nloop_headers.insert(
                        0, (index_id, lbound_exp, ubound_exp, stride_exp)
                    )
                    ipos = ivar_names.index(index_id.name)
    ...
    def __getARefInfo(self):
        ...
        if aref == None:
            err(
                'orio.module.loop.submodule.arrcopy.transformation:  array-copy statement does not contain array reference: "%s"'
                % self.aref
            )
        ...
        for e in dim_exps:
            ...
            if len(c_inames) != 1:
                err(
                    (
                        "orio.module.loop.submodule.arrcopy.transformation: each dimension expression of array reference %s must contain "
                        + "exactly one iteration variable"
                    )
                    % self.aref
                )
            iname = c_inames[0]
            if iname in ivar_names:
                err(
                    'orio.module.loop.submodule.arrcopy.transformation: repeated iteration variable name "%s" in array reference %s'
                    % (iname, aref)
                )
        ...
        if len(self.dimsizes) != len(dim_exps):
            err(
                "orio.module.loop.submodule.arrcopy.transformation:  incorrect the number of array dimensions: %s"
                % self.dimsizes
            )
