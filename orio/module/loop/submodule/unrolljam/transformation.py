...
class Transformation:
    ...
    def __init__(self, ufactor, do_jamming, stmt, parallelize):
        ...
        self.newVarsOp = set(
            []
        )  # tells you which operation is operated on the new set of variables (either none, plus, or multiply)
        self.varsToAdd = set(
            []
        )  # tells you which variable names considered for introduction. Need ufactor to deduce the full set of variables introduced.
        self.varsNoAdd = set([])
        self.localvars = set([])
    ...
    def __computeNewVarsIntro(self, tnode):
        ...
        binOpExprs = self.__analyzeForNewVars(
            tnode
        )  # has the form set([ lfsName, (rhsName1, rhsName2, ...)]
        ...
        if len(self.newVarsOp) != 1 or (
            iter(self.newVarsOp).next() != orio.module.loop.ast.BinOpExp.ADD
            and iter(self.newVarsOp).next() != orio.module.loop.ast.BinOpExp.MUL
        ):
            ...
            if exp1[0] in exp1[1]:  # there's modification of variable
                for exp2 in binOpExprs:
                    if exp2 is exp1:
                        continue
                    if (
                        exp1[0] in exp2[1] and exp1[0] != exp2[0]
                    ):  # modification with alias
        ...
        if isinstance(tnode, orio.module.loop.ast.ExpStmt):
            ...
            if not isinstance(tnode.lhs, orio.module.loop.ast.IdentExp):
                if not isinstance(tnode.lhs, orio.module.loop.ast.ArrayRefExp):
                    err(
                        "left hand side of a top level binary op expression is not an identifer expression or array ref exp???"
                    )
            else:
            ...
            if not isinstance(tnode.exp, orio.module.loop.ast.IdentExp):
                if not isinstance(tnode.exp, orio.module.loop.ast.ArrayRefExp):
                    err(
                        "the sub level exp of top level unary expression is not an identifer expression or array ref exp???"
                    )
            else:
            ...
            err(
                'orio.module.loop.submodule.unrolljam.transformation.__analyzeForNewVars internal error: unexpected AST type: "%s"'
                % tnode.__class__.__name__
            )
    ...
    def __extractFromBinOp(self, tnode):
        ...
        if isinstance(tnode, orio.module.loop.ast.ParenthExp):
            ...
            if isinstance(tnode.lhs, orio.module.loop.ast.ParenthExp):
                ...
                var_names = reduce(
                    set.union, map(self.__extractFromBinOp, tnode.lhs.args), var_names
                )
            elif isinstance(tnode.lhs, orio.module.loop.ast.NumLitExp):
                pass
            else:
                err(
                    'orio.module.loop.submodule.unrolljam.transformation.__extractFromBinOp.BinOpExp internal error: unexpected AST type: "%s", lhs: %s'
                    % (tnode.lhs.__class__.__name__, tnode.lhs)
                )
            ...
            if isinstance(tnode.rhs, orio.module.loop.ast.ParenthExp):
                ...
                var_names = reduce(
                    set.union, map(self.__extractFromBinOp, tnode.rhs.args), var_names
                )
            elif isinstance(tnode.rhs, orio.module.loop.ast.NumLitExp):
                pass
            else:
                err(
                    'orio.module.loop.submodule.unrolljam.transformation.__extractFromBinOp internal error: unexpected AST type: "%s", rhs: %s'
                    % (tnode.rhs.__class__.__name__, tnode.rhs)
                )

            self.newVarsOp |= set([tnode.op_type])
        else:
            err(
                'orio.module.loop.submodule.unrolljam.transformation.__extractFromBinOp. internal error: unexpected AST type: "%s", node: %s'
                % (tnode.__class__.__name__, tnode)
            )
        ...
        for i in range(num):
            ...
            for stmts in stmtss:
                if s1 == None:
                    ...
                    s2 = stmts[i]
                    assert isinstance(
                        s2, orio.module.loop.ast.ForStmt
                    ), "internal error: not a loop statement"
                    if not (
                        str(s1.init) == str(s2.init)
                        and str(s1.test) == str(s2.test)
                        and str(s1.iter) == str(s2.iter)
                    ):
                        is_jam_valid = False
        if is_jam_valid:
            if not contain_loop:
    ...
    def transform(self):
        ...
        while (
            isinstance(self.stmt.stmt, orio.module.loop.ast.CompStmt)
            and len(self.stmt.stmt.stmts) == 1
        ):
        ...
        if not index_id.name in self.localvars:
            ...
            debug(
                "loop.unrolljam.transformation: generating var decl: %s %s"
                % (int, index_id.name),
                self,
            )
        ...
        if self.ufactor == 1:
            orig_loop = self.flib.createForLoop(
                index_id, lbound_exp, ubound_exp, stride_exp, loop_body, meta="original"
            )
            if self.parallelize:
        ...
        it = orio.module.loop.ast.BinOpExp(
            ...
            orio.module.loop.ast.NumLitExp(
                self.ufactor - 1, orio.module.loop.ast.NumLitExp.INT
            ),
        ...
        it = orio.module.loop.ast.NumLitExp(
            self.ufactor, orio.module.loop.ast.NumLitExp.INT
        )
        ...
        lbound_init = orio.module.loop.ast.VarDeclInit(
            "int", lbound_name_exp, new_lbound_exp
        )
