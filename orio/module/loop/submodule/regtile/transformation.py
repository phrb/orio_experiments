...
class Transformation:
    ...
    def __init__(self, loops, ufactors, stmt):
        ...
        self.itvar_map = dict(
            zip(self.loops, [l + self.__ivar_suffix for l in self.loops])
        )
        self.itvar_map_rev = dict(
            zip([l + self.__ivar_suffix for l in self.loops], self.loops)
        )
    ...
    def __addIdentWithExp(self, tnode, index_name, exp):
        ...
        if isinstance(exp, orio.module.loop.ast.NumLitExp) and exp.val == 0:
            ...
            err(
                "orio.module.loop.submodule.regtile.transformation internal error: unprocessed transform statement"
            )
            ...
            err(
                'orio.module.loop.submodule.regtile.transformation internal error: unexpected AST type: "%s"'
                % tnode.__class__.__name__
            )

    # ----------------------------------------------------------

    def __createIntraTileLoop(
        self, intratile_iname, intertile_iname, ufactor, stride_exp, loop_body
    ):
        ...
        it = orio.module.loop.ast.BinOpExp(
            ...
            orio.module.loop.ast.NumLitExp(
                ufactor - 1, orio.module.loop.ast.NumLitExp.INT
            ),
            orio.module.loop.ast.BinOpExp.MUL,
        )
        ubound_exp = orio.module.loop.ast.BinOpExp(
            orio.module.loop.ast.IdentExp(intertile_iname),
            it,
        ...
        intratile_loop = self.flib.createForLoop(
            index_id, lbound_exp, ubound_exp, stride_exp, loop_body.replicate()
        )
    ...
    def __createUnrolledLoops(self, for_loop_info, ufactor):
        ...
        if lbound_exp == None or ubound_exp == None or stride_exp == None:
            err(
                "orio.module.loop.submodule.regtile.transformation: the loop to be unrolled/jammed must NOT have empty bounds/stride "
                + "expressions"
            )
        ...
        mloop_body = self.clib.replaceIdent(
            loop_body.replicate(), cloop_index_id.name, mloop_index_id.name
        )

        # generate the orio.main.unrolled loop
        loop = self.flib.createForLoop(
            mloop_index_id, new_lbound_exp, new_ubound_exp, new_stride_exp, mloop_body
        )

        # generate the cleanup loop
        cleanup_loop = self.flib.createForLoop(
            cloop_index_id, mloop_index_id, ubound_exp, stride_exp, loop_body
        )
        ...
        if isinstance(stmt, orio.module.loop.ast.ExpStmt):
            ...
            for iname, ufactor, stride_exp in outer_unrolls_rev:
                ...
                for i in range(0, ufactor):
                    ...
                    n_stmts += [
                        self.__addIdentWithExp(s.replicate(), iname, increment_exp)
                        for s in stmts
                    ]
                stmts = n_stmts
            ...
            for iname, ufactor, stride_exp in bound_unrolls:
                ...
                # to create an intra-tile loop (not to fully unroll the unjammable statement)
                    ...
                    nstmt = self.__createIntraTileLoop(
                        intratile_iname, iname, ufactor, stride_exp, loop_body
                    )
                    stmts = [nstmt]
            ...
            i_index_id, i_lbound_exp, i_ubound_exp, i_stride_exp, i_loop_body = (
                i_for_loop_info
            )
            ...
            for u in outer_unrolls:
                ...
                if ufactor == 1:
                    continue
                if (
                    self.clib.containIdentName(i_lbound_exp, iname)
                    or self.clib.containIdentName(i_ubound_exp, iname)
                    or self.clib.containIdentName(i_stride_exp, iname)
                ):
                    bound_unrolls.append(u)
                else:
                    free_unrolls.append(u)
                ...
                if self.use_full_unroll:
                    ...
                    for iname, ufactor, stride_exp in bound_unrolls:
                        ...
                        for i in range(0, ufactor):
                            ...
                            for s in stmts:
                                n_stmts.append(
                                    self.__addIdentWithExp(
                                        s.replicate(), iname, increment_exp
                                    )
                                )
                        stmts = n_stmts
                    if len(stmts) == 1:
                    ...
                    for iname, ufactor, stride_exp in bound_unrolls:
                        intratile_iname = self.itvar_map_rev[iname]
                        loop_body = self.clib.replaceIdent(
                            nstmt, iname, intratile_iname
                        )
                        nstmt = self.__createIntraTileLoop(
                            intratile_iname, iname, ufactor, stride_exp, loop_body
                        )
            ...
            # to apply the simple (rectangular) register tiling to this loop
                ...
                loop, cleanup_loop = self.__createUnrolledLoops(
                    i_for_loop_info, i_ufactor
                )
                loop.stmt = self.__transformLoop(loop.stmt, ext_free_unrolls)
                cleanup_loop.stmt = self.__transformLoop(
                    cleanup_loop.stmt, orig_free_unrolls
                )
                bound_unrolls.reverse()
                if self.use_full_unroll:
            ...
            # to apply the non-rectangular register tiling to this loop
                ...
                new_lb_id = orio.module.loop.ast.IdentExp(
                    self.__newlb_prefix + i_index_id.name
                )
                new_ub_id = orio.module.loop.ast.IdentExp(
                    self.__newub_prefix + i_index_id.name
                )
                ...
                if len(lb_bound_unrolls_rev) == 0:
                    ...
                    a_val = orio.module.loop.ast.NumLitExp(
                        self.__int_min, orio.module.loop.ast.NumLitExp.INT
                    )
                lb_asgn = orio.module.loop.ast.BinOpExp(
                    new_lb_id.replicate(), a_val, orio.module.loop.ast.BinOpExp.EQ_ASGN
                )
                ...
                if len(ub_bound_unrolls_rev) == 0:
                    ...
                    a_val = orio.module.loop.ast.NumLitExp(
                        self.__int_max, orio.module.loop.ast.NumLitExp.INT
                    )
                ub_asgn = orio.module.loop.ast.BinOpExp(
                    new_ub_id.replicate(), a_val, orio.module.loop.ast.BinOpExp.EQ_ASGN
                )
                ...
                rhs = orio.module.loop.ast.FunCallExp(
                    orio.module.loop.ast.IdentExp("max"),
                    [new_lb_id.replicate(), i_lbound_exp.replicate()],
                )
                lb_asgn = orio.module.loop.ast.BinOpExp(
                    lhs, rhs, orio.module.loop.ast.BinOpExp.EQ_ASGN
                )
                ...
                rhs = orio.module.loop.ast.FunCallExp(
                    orio.module.loop.ast.IdentExp("min"),
                ...
                if len(stmts) == 0:
                    err(
                        "orio.module.loop.submodule.regtile.transformation internal error: loop-bound initialization loop cannot be empty"
                    )
                if len(stmts) == 1:
                    bound_stmt = stmts[0]
                else:
                ...
                for iname, ufactor, stride_exp in bound_unrolls_rev:
                    intratile_iname = self.itvar_map_rev[iname]
                    loop_body = self.clib.replaceIdent(
                        bound_stmt, iname, intratile_iname
                    )
                    bound_stmt = self.__createIntraTileLoop(
                        intratile_iname, iname, ufactor, stride_exp, loop_body
                    )

                # create the statement for computing the new lower/upper bounds
                bound_stmt = orio.module.loop.ast.CompStmt(
                    [lb_init, ub_init, bound_stmt]
                )
                ...
                loop, cleanup_loop = self.__createUnrolledLoops(
                    c_for_loop_info, i_ufactor
                )
                loop.stmt = self.__transformLoop(loop.stmt, ext_outer_unrolls)
                cleanup_loop.stmt = self.__transformLoop(
                    cleanup_loop.stmt, orig_outer_unrolls
                )

                # create the prologue and epilogue loops
                ub_exp = orio.module.loop.ast.BinOpExp(
                    new_lb_id.replicate(),
                    i_stride_exp.replicate(),
                    orio.module.loop.ast.BinOpExp.SUB,
                )
                prol_loop = self.flib.createForLoop(
                    i_index_id, i_lbound_exp, ub_exp, i_stride_exp, i_loop_body
                )
                prol_loop.stmt = self.__transformLoop(prol_loop.stmt, orig_free_unrolls)
                lb_exp = orio.module.loop.ast.BinOpExp(
                    new_ub_id.replicate(),
                    i_stride_exp.replicate(),
                    orio.module.loop.ast.BinOpExp.ADD,
                )
                epil_loop = self.flib.createForLoop(
                    i_index_id, lb_exp, i_ubound_exp, i_stride_exp, i_loop_body
                )
                ...
                if self.use_full_unroll:
                    ...
                    for iname, ufactor, stride_exp in bound_unrolls_rev:
                        intratile_iname = self.itvar_map_rev[iname]
                        prol_loop_body = self.clib.replaceIdent(
                            prol_stmt, iname, intratile_iname
                        )
                        epil_loop_body = self.clib.replaceIdent(
                            epil_stmt, iname, intratile_iname
                        )
                        prol_stmt = self.__createIntraTileLoop(
                            intratile_iname, iname, ufactor, stride_exp, prol_loop_body
                        )
                        epil_stmt = self.__createIntraTileLoop(
                            intratile_iname, iname, ufactor, stride_exp, epil_loop_body
                        )
