    ...
    def __checkIdenticalNestedLoop(self, stmt, outer_loops):
        ...
        if isinstance(stmt, orio.module.loop.ast.ExpStmt):
            ...
            if index_id.name in outer_loops:
                err(
                    'orio.module.loop.submodule.regtile.semant: loops with the same iteration name "%s" cannot be nested'
                    % index_id.name
                )
            ...
            err(
                "orio.module.loop.submodule.regtile.semant internal error: unprocessed transform statement"
            )
            ...
            err(
                'orio.module.loop.submodule.regtile.semant internal error: unexpected AST type: "%s"'
