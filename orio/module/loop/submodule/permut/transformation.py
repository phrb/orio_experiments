...
class Transformation:
    ...
    def checkPermutSeq(self, stmt):
        ...
        for i in self.seq:
            if isinstance(i, str) and i not in inames:
                err(
                    'orio.module.loop.submodule.permut.transformation:  loop permutation with index name "%s" does not exist'
                    % i
                )
    ...
    def __getPermutNests(self, stmt):
        ...
        for n in permut_nests:
            inames = []
            for l in n:
                index_id, lbound, ubound, stride, lbody = self.flib.extractForLoopInfo(
                    l
                )
    ...
    def __collectPermutNests(self, stmt, pnests):
        ...
        if isinstance(stmt, orio.module.loop.ast.ExpStmt):
            ...
            err(
                "orio.module.loop.submodule.permut.transformation internal error: unprocessed transform statement"
            )
            ...
            err(
                'orio.module.loop.submodule.permut.transformation internal error: unexpected AST type: "%s"'
    ...
    def transform(self):
        ...
        if not permut_executed:
            err(
                "orio.module.loop.submodule.permut.transformation:  permutation cannot be executed. please check the sequence: %s"
                % self.seq
            )
