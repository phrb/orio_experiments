...
class Transformation:
    ...
    def transform(self):
        ...
        decls = (
            [VarDeclInit("int", self.cs["tag"], self.cs["int0"])]
            + [VarDecl("MPI_Request ", [requestrecv + "[" + numcomm + "]"])]
            + [VarDecl("int", ["neighbor" + "[" + numcomm + "]"])]
            + [VarDecl("MPI_Request ", [requestsend + "[" + numcomm + "]"])]
            + [VarDecl("int", ["sindex"])]
            + [VarDecl("int", ["rindex"])]
            + [VarDecl("MPI_Status", ["statuss" + "[" + numcomm + "]"])]
            + [VarDecl("MPI_Status", ["statusr" + "[" + numcomm + "]"])]
            + [VarDecl("int", ["numneighbour"])]
        )
        ...
        if self.communication == 1:
            setnumneighbors = [
                ExpStmt(
                    BinOpExp(self.cs["nneighbour"], self.cs["int1"], BinOpExp.EQ_ASGN)
                )
            ]
            setneighbors = [
                IfStmt(
                    BinOpExp(
                        BinOpExp(self.cs["nprocs"], self.cs["int2"], BinOpExp.MOD),
                        self.cs["int0"],
                        BinOpExp.EQ,
                    ),
                    CompStmt(
                        [
                            ...
                            IfStmt(
                                BinOpExp(self.cs["id"], self.cs["int0"], BinOpExp.EQ),
                                ExpStmt(
                                    BinOpExp(
                                        self.cs["lneighbor"],
                                        BinOpExp(
                                            self.cs["id"], self.cs["int1"], BinOpExp.ADD
                                        ),
                                        BinOpExp.EQ_ASGN,
                                    )
                                ),
                    ...
                    IfStmt(
                        ...
                        IfStmt(
                            BinOpExp(
                                self.cs["id"],
                                BinOpExp(
                                    self.cs["nprocs"], self.cs["int1"], BinOpExp.SUB
                                ),
                                BinOpExp.EQ,
                            ),
                            ExpStmt(
                            ...
                            IfStmt(
                                BinOpExp(
                                    BinOpExp(
                                        self.cs["id"], self.cs["int2"], BinOpExp.MOD
                                    ),
                                    self.cs["int0"],
                                    BinOpExp.EQ,
                                ),
                                ...
                                ExpStmt(
                                    BinOpExp(
                                        self.cs["lneighbor"],
                                        BinOpExp(
                                            self.cs["id"], self.cs["int1"], BinOpExp.SUB
                                        ),
                                        BinOpExp.EQ_ASGN,
                                    )
                                ),
            ...
            setnumneighbors = [
                ExpStmt(
                    BinOpExp(self.cs["nneighbour"], self.cs["int2"], BinOpExp.EQ_ASGN)
                )
            ]
            setneighbors = [
                IfStmt(
                    ...
                    IfStmt(
                        BinOpExp(
                            self.cs["id"],
                            BinOpExp(self.cs["nprocs"], self.cs["int1"], BinOpExp.SUB),
                            BinOpExp.EQ,
                        ),
                        CompStmt(
                            [
            ...
            setnumneighbors = [
                IfStmt(
                    ...
                    ExpStmt(
                        BinOpExp(
                            self.cs["nneighbour"],
                            BinOpExp(self.cs["nprocs"], self.cs["int2"], BinOpExp.DIV),
                            BinOpExp.EQ_ASGN,
                        )
                    ),
                    IfStmt(
                        BinOpExp(
                            BinOpExp(self.cs["id"], self.cs["int2"], BinOpExp.MOD),
                            self.cs["int0"],
                            BinOpExp.EQ,
                        ),
                        ExpStmt(
                            BinOpExp(
                                self.cs["nneighbour"],
                                BinOpExp(
                                    self.cs["nprocs"], self.cs["int2"], BinOpExp.DIV
                                ),
                                BinOpExp.EQ_ASGN,
                            )
                        ),
                ...
                IfStmt(
                    ...
                    CompStmt(
                        [
                            ExpStmt(
                                BinOpExp(
                                    self.cs["j"], self.cs["int0"], BinOpExp.EQ_ASGN
                                )
                            ),
                            ForStmt(
                                BinOpExp(
                                    self.cs["i"], self.cs["int1"], BinOpExp.EQ_ASGN
                                ),
                                BinOpExp(self.cs["i"], self.cs["nprocs"], BinOpExp.LT),
                                BinOpExp(
                                    self.cs["i"],
                                    BinOpExp(
                                        self.cs["i"], self.cs["int2"], BinOpExp.ADD
                                    ),
                                    BinOpExp.EQ_ASGN,
                                ),
                                CompStmt(
                                    [
                                        ExpStmt(
                                            BinOpExp(
                                                self.cs["commneighborj"],
                                                self.cs["i"],
                                                BinOpExp.EQ_ASGN,
                                            )
                                        ),
                                        ExpStmt(
                                            UnaryExp(self.cs["j"], UnaryExp.POST_INC)
                                        ),
                                    ]
                                ),
                            ),
                    ...
                    CompStmt(
                        [
                            ...
                            ForStmt(
                                BinOpExp(
                                    self.cs["i"], self.cs["int0"], BinOpExp.EQ_ASGN
                                ),
                                BinOpExp(self.cs["i"], self.cs["nprocs"], BinOpExp.LT),
                                BinOpExp(
                                    self.cs["i"],
        ...
        requestr = UnaryExp(
            ArrayRefExp(IdentExp("requestr"), IdentExp("i")), UnaryExp.ADDRESSOF
        )

        requests = UnaryExp(
            ArrayRefExp(IdentExp("requests"), IdentExp("i")), UnaryExp.ADDRESSOF
        )

        if self.protocol == "rendezvous":
            comm = [
                ForStmt(
                    BinOpExp(self.cs["i"], self.cs["int0"], BinOpExp.EQ_ASGN),
                        ...
                        [
                            ExpStmt(
                                FunCallExp(
                                    ...
                                    [
                                        ArrayRefExp(
                                            IdentExp("recvbuff"), IdentExp("i")
                                        ),
                                        self.cs["maxmsg"],
                                        self.cs["dtype"],
                                        self.cs["commneighbor"],
                                        self.cs["tag"],
                                        self.cs["commworld"],
                                        requestr,
                                    ],
                ...
                ForStmt(
                    ...
                    ExpStmt(
                        FunCallExp(
                            IdentExp("MPI_Irecv"),
                            [
                                ArrayRefExp(IdentExp("recvbuff"), IdentExp("i")),
                                self.cs["maxmsg"],
                                self.cs["dtype"],
                                self.cs["commneighbor"],
                                self.cs["tag"],
            ...
            barr = [
                ExpStmt(FunCallExp(IdentExp("MPI_Barrier"), [self.cs["commworld"]]))
            ]
                ...
                ForStmt(
                    ...
                    ExpStmt(
                        FunCallExp(
                            IdentExp("MPI_Irsend"),
                            [
                                ArrayRefExp(IdentExp("sendbuff"), IdentExp("i")),
                                self.cs["maxmsg"],
                                self.cs["dtype"],
                                self.cs["commneighbor"],
                                self.cs["tag"],
            ...
            ForStmt(
                ...
                CompStmt(
                    [
                        ExpStmt(
                            FunCallExp(
                                IdentExp("MPI_Waitany"),
                                [
                                    self.cs["nneighbour"],
                                    IdentExp("requestr"),
                                    UnaryExp(self.cs["rindex"], UnaryExp.ADDRESSOF),
                                    IdentExp("statusr"),
                                ],
                            )
                        ),
                        ExpStmt(
                            FunCallExp(
                                ...
                                [
                                    self.cs["nneighbour"],
                                    IdentExp("requests"),
                                    UnaryExp(self.cs["sindex"], UnaryExp.ADDRESSOF),
                                    IdentExp("statuss"),
                                ],
                            )
                        ),
        ...
        transformed_stmt = orio.module.loop.ast.CompStmt(
            ...
            + [
                ForStmt(
                    ...
                    UnaryExp(self.cs["i"], UnaryExp.POST_INC),
                    CompStmt(self.newstmts["comm"] + stmts + self.newstmts["wait"]),
                )
            ]
        )
