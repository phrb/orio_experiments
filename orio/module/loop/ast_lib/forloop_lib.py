...
class ForLoopLib:
    ...
    def extractForLoopInfo(self, stmt):
        ...
        if not isinstance(stmt, orio.module.loop.ast.ForStmt):
            err(
                "orio.module.loop.ast_lib.forloop_lib: %s: not a for-loop statement"
                % stmt.line_no
            )
        ...
        if stmt.init:
            while True:
                ...
                if (
                    isinstance(stmt.init, orio.module.loop.ast.BinOpExp)
                    and stmt.init.op_type == orio.module.loop.ast.BinOpExp.EQ_ASGN
                ):
                    ...
                    if isinstance(stmt.init.lhs, orio.module.loop.ast.IdentExp):
                        break
                err(
                    'orio.module.loop.ast_lib.forloop_lib:%s: loop initialization expression not in "<id> = <exp>" form'
                    % stmt.init.line_no
                )
        ...
        if stmt.test:
            while True:
                ...
                if (
                    isinstance(stmt.test, orio.module.loop.ast.BinOpExp)
                    and
                    # (
                    ...
                    if isinstance(stmt.test.lhs, orio.module.loop.ast.IdentExp):
                        break
                err(
                    'orio.module.loop.ast_lib.forloop_lib:%s: loop test expression not in "<id> <= <exp>" form'
                    % stmt.test.line_no
                )
        ...
        if stmt.iter:
            while True:
                ...
                if (
                    isinstance(stmt.iter, orio.module.loop.ast.BinOpExp)
                    and stmt.iter.op_type == orio.module.loop.ast.BinOpExp.EQ_ASGN
                ):
                    ...
                    if isinstance(stmt.iter.lhs, orio.module.loop.ast.IdentExp):
                        if isinstance(
                            stmt.iter.rhs, orio.module.loop.ast.BinOpExp
                        ) and stmt.iter.rhs.op_type in (
                            orio.module.loop.ast.BinOpExp.ADD,
                            orio.module.loop.ast.BinOpExp.SUB,
                        ):
                            while isinstance(
                                stmt.iter.rhs.lhs, orio.module.loop.ast.ParenthExp
                            ):
                                stmt.iter.rhs.lhs = stmt.iter.rhs.lhs.exp
                            while isinstance(
                                stmt.iter.rhs.rhs, orio.module.loop.ast.ParenthExp
                            ):
                                stmt.iter.rhs.rhs = stmt.iter.rhs.rhs.exp
                            if (
                                isinstance(
                                    stmt.iter.rhs.lhs, orio.module.loop.ast.IdentExp
                                )
                                and stmt.iter.lhs.name == stmt.iter.rhs.lhs.name
                            ):
                                break
                elif isinstance(
                    stmt.iter, orio.module.loop.ast.UnaryExp
                ) and stmt.iter.op_type in (
                    orio.module.loop.ast.UnaryExp.POST_INC,
                ...
                err(
                    (
                        'orio.module.loop.ast_lib.forloop_lib:%s: loop iteration expression not in "<id>++" or "<id>--" or '
        ...
        if not stmt.init and not stmt.test and not stmt.iter:
            err(
                "orio.module.loop.ast_lib.forloop_lib:%s: a loop with an empty control expression cannot be handled"
                % stmt.line_no
            )
        ...
        if inames.count(inames[0]) != len(inames):
            err(
                "orio.module.loop.ast_lib.forloop_lib:%s: iterator names across init, test, and iter exps must be the same"
                % stmt.line_no
            )
        ...
        if stmt.iter:
            if isinstance(stmt.iter, orio.module.loop.ast.BinOpExp):
                ...
                if stmt.iter.rhs.op_type == orio.module.loop.ast.BinOpExp.SUB:
                    stride_exp = orio.module.loop.ast.UnaryExp(
                        stride_exp, orio.module.loop.ast.UnaryExp.MINUS
                    )
            elif isinstance(stmt.iter, orio.module.loop.ast.UnaryExp):
                if stmt.iter.op_type in (
                    orio.module.loop.ast.UnaryExp.POST_INC,
                    orio.module.loop.ast.UnaryExp.PRE_INC,
                ):
                    stride_exp = orio.module.loop.ast.NumLitExp(
                        1, orio.module.loop.ast.NumLitExp.INT
                    )
                elif stmt.iter.op_type in (
                    orio.module.loop.ast.UnaryExp.POST_DEC,
                    orio.module.loop.ast.UnaryExp.PRE_DEC,
                ):
                    stride_exp = orio.module.loop.ast.NumLitExp(
                    ...
                    err(
                        "orio.module.loop.ast_lib.forloop_lib internal error: unexpected unary operation type"
                    )
            else:
                err(
                    "orio.module.loop.ast_lib.forloop_lib internal error: unexpected type of iteration expression"
                )
    ...
    def createForLoop(
        self, index_id, lbound_exp, ubound_exp, stride_exp, loop_body, meta=""
    ):
        """
        Generate a for loop:
        ...
        iter_exp = None
        if lbound_exp:
            init_exp = orio.module.loop.ast.BinOpExp(
                index_id.replicate(),
                lbound_exp.replicate(),
                orio.module.loop.ast.BinOpExp.EQ_ASGN,
            )
        if ubound_exp:
            test_exp = orio.module.loop.ast.BinOpExp(
                index_id.replicate(),
        ...
        if stride_exp:
            ...
            iter_exp = orio.module.loop.ast.BinOpExp(
                index_id.replicate(), it, orio.module.loop.ast.BinOpExp.EQ_ASGN
            )
        return orio.module.loop.ast.ForStmt(
            init_exp, test_exp, iter_exp, loop_body.replicate(), meta={"kind": meta}
        )
    ...
    def getLoopIndexNames(self, stmt):
        ...
        if isinstance(stmt, orio.module.loop.ast.ExpStmt):
            ...
            index_id, lbound_exp, ubound_exp, stride_exp, loop_body = self.extractForLoopInfo(
                stmt
            )
            ...
            err(
                "orio.module.loop.ast_lib.forloop_lib internal error: unprocessed transform statement"
            )
            ...
            err(
                'orio.module.loop.ast_lib.forloop_lib internal error: unexpected AST type: "%s"'
