...
def t_error(t):
    err(
        'orio.module.loop.parser: %s: syntactical error: "%s"'
        % ((t.lineno + __start_line_no - 1), t.value[0])
    )
...
def p_iteration_statement(p):
    ...
    p[0] = ast.ForStmt(
        p[3], p[5], p[7], p[9], line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_transformation_statement(p):
    ...
    p[0] = ast.TransformStmt(
        p[2], p[4], p[6], line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_transformation_statement2(p):
    ...
    p[0] = ast.TransformStmt(
        p[2], p[4], None, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_expression_2(p):
    ...
    p[0] = ast.BinOpExp(
        p[1], p[3], ast.BinOpExp.COMMA, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_expression_4(p):
    ...
    p[0] = ast.VarDeclInit(
        p[1], ast.IdentExp(p[2]), p[4], line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_assignment_expression_2(p):
    ...
    if p[2] == "=":
        p[0] = ast.BinOpExp(
            ...
            line_no=str(p.lineno(1) + __start_line_no - 1),
        )
    elif p[2] in ("*=", "/=", "%=", "+=", "-="):
        lhs = p[1].replicate()
        rhs = None
        if p[2] == "*=":
            ...
            err(
                "orio.module.loop.parser internal error: missing case for assignment operator"
            )
        p[0] = ast.BinOpExp(
            lhs,
            rhs,
...
def p_logical_or_expression_2(p):
    ...
    p[0] = ast.BinOpExp(
        p[1], p[3], ast.BinOpExp.LOR, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_logical_and_expression_2(p):
    ...
    p[0] = ast.BinOpExp(
        p[1], p[3], ast.BinOpExp.LAND, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_equality_expression_2(p):
    ...
    if p[2] == "==":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.EQ, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    elif p[2] == "!=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.NE, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    else:
...
def p_relational_expression_2(p):
    ...
    if p[2] == "<":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.LT, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    elif p[2] == ">":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.GT, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    elif p[2] == "<=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.LE, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    elif p[2] == ">=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.GE, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    else:
...
def p_additive_expression_2(p):
    ...
    if p[2] == "+":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.ADD, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    elif p[2] == "-":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.SUB, line_no=str(p.lineno(1) + __start_line_no - 1)
        )
    else:
...
def p_postfix_expression_5(p):
    ...
    p[0] = ast.UnaryExp(
        p[1], ast.UnaryExp.POST_DEC, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_primary_expression_2(p):
    ...
    val = int(p[1])
    p[0] = ast.NumLitExp(
        val, ast.NumLitExp.INT, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_primary_expression_3(p):
    ...
    val = float(p[1])
    p[0] = ast.NumLitExp(
        val, ast.NumLitExp.FLOAT, line_no=str(p.lineno(1) + __start_line_no - 1)
    )
...
def p_error(p):
    ...
    pos = (col - 1) * " "
    err(
        "[orio.module.loop.parser] unexpected symbol '%s' at line %s, column %s:\n\t%s\n\t%s^"
