        ...
        if isinstance(stmt, ast.ExpStmt):
            ...
            for [aname, _, line_no] in stmt.args:
                if aname in arg_names:
                    g.err(
                        __name__
                        + ': %s: repeated transformation argument: "%s"'
                        % (line_no, aname)
                    )
            ...
            submod_name = ".".join(
                [TSUBMOD_NAME, class_name.lower(), class_name.lower()]
            )
            submod_class = self.dloader.loadClass(submod_name, class_name)

            # apply code transformations
            t = submod_class(
                self.perf_params, stmt.args, stmt.stmt, self.language, self.tinfo
            )
            ...
            g.err(
                __name__
                + ": internal error: unknown statement type: %s"
