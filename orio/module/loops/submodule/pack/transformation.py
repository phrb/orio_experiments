...
class Transformation:
    ...
    def transform(self):
        ...
        # find array ids
        getArrayIds = (
            lambda n: nv.collectTD(getIds, n.exp) if isinstance(n, ArrayRefExp) else []
        )
        ...
        getLhsIds = (
            lambda n: nv.collectTD(getIds, n.lhs)
            if isinstance(n, BinOpExp) and n.op_type in assignOps
            else []
        )
        getRhsIds = (
            lambda n: nv.collectTD(getIds, n.rhs)
            if isinstance(n, BinOpExp) and n.op_type == BinOpExp.EQ
            else nv.collectTD(getIds, n)
            if isinstance(n, BinOpExp) and n.op_type in shortAssignOps
            else []
        )
        ...
        def addPrefetch(id):
            ...
            def rw(n):
                if isinstance(n, ExpStmt):
                    existsId = nv.collectTD(
                        lambda a: [(True, a.sub)]
                        if isinstance(a, ArrayRefExp) and a.exp.name == id
                        else [],
                        n,
                    )
                    if len(existsId) > 0:
                        for atuple in existsId:
                            atuple2 = atuple[1]
                            idxId = IdentExp("")
                            if isinstance(atuple2, UnaryExp) and isinstance(
                                atuple2.exp, IdentExp
                            ):
                                idxId = atuple2.exp
                                        ...
                                        UnaryExp(
                                            ...
                                            ArrayRefExp(
                                                ...
                                                BinOpExp(
                                                    BinOpExp.PLUS,
                                                    idxId,
                                                    IdentExp(self.prefetch_distance),
                                                ),
                                            ),
                                        ),
                                        LitExp(LitExp.INT, 0),
