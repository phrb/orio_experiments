...
class Variable:
    ...
    def semantCheck(self):
        ...
        if len(self.dims) == 0:
            err(
                'orio.module.align.variable: %s: at least one dimension must be defined: "%s"'
                % (self.line_no, self)
            )
        ...
        if self.dims.count(None) != 1:
            err(
                'orio.module.align.variable: %s: there must be one empty bracket: "%s"'
                % (self.line_no, self)
            )
        ...
        if self.dims[-1] != None:
            err(
                (
                    "orio.module.align.variable:%s: the last dimension must be an empty bracket "
                    + '(due to the row-major array allocation): "%s"'
                )
                % (self.line_no, self)
            )
