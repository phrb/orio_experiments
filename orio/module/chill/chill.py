    ...
    def transform(self):
        ...
        */
        ...
        for i in range(stm):
            ...
            temp = [
                self.perf_params["TX" + str(i)],
                self.perf_params["TY" + str(i)],
                self.perf_params["BX" + str(i)],
                self.perf_params["BY" + str(i)],
            ]
        ...
        if cont == True:
            ...
            for line in annot:
                ...
                if lineInfo[0] == "unroll":
                    ...
                    for l in range(len(loopsNumber[stmPos])):
                        loopsInfo = filter(
                            None, re.split(" |\(|\)|;", loopsNumber[stmPos][l])
                        )
            ...
            for line in fname:
                ...
                lineInfo = filter(
                    None,
                    re.split(
                        " |(double|cudaMalloc|cudaMemcpy|dim|cudaFree|<<<|__global__|;)",
                        line,
                    ),
                )
            ...
            newHost += (
                "  time = (1000000.0*(time2.tv_sec-time1.tv_sec) + time2.tv_usec-time1.tv_usec)/1000000.0;\n"
            )
            newHost += (
                '  timefile.open("./times/time_'
                + stamp
                + '.txt", std::ofstream::out | std::ofstream::app );\n'
            )
            newHost += (
                '  timefile<<"Time spent in rose__orio_chill_ '
                + stamp
                + '.cu: "<<time<<std::endl;\n'
            )
            ...
            for cuda in cudaKernels:
                ...
                for loops in fors:
                    ...
                    if regs[1] + " +=" in loops:
                        newHost += (
                            space + "newVar" + str(var) + " = " + copyReg[0] + ";\n"
                        )
                        par = ""
                        closing = (
                            space
            ...
            try:
                os.system(cmd)
                cmd = (
                    "mv rose__orio_chill_.cu outputs/rose__orio_chill_" + stamp + ".cu"
                )
                os.system(cmd)
            except:
