...
class ASTUtil:
    ...
    def containIdentName(self, exp, iname):
        ...
        if not isinstance(exp, ast.Exp):
            err(
                "orio.module.ortildriver.ast_util internal error:containIdentName: input not an expression type"
            )
        ...
        if isinstance(exp, ast.NumLitExp):
            ...
            return self.containIdentName(exp.exp, iname) or self.containIdentName(
                exp.sub_exp, iname
            )
            ...
            return self.containIdentName(exp.lhs, iname) or self.containIdentName(
                exp.rhs, iname
            )
            ...
            err(
                'orio.module.ortildriver.ast_util internal error: unexpected AST type: "%s"'
    ...
    def getForLoopInfo(self, stmt):
        ...
        if not isinstance(stmt, ast.ForStmt):
            err(
                "orio.module.ortildriver.ast_util: OrTilDriver:%s: not a for-loop statement"
                % stmt.line_no
            )
        ...
        if stmt.init:
            while True:
                ...
                if (
                    isinstance(stmt.init, ast.BinOpExp)
                    ...
                    if isinstance(stmt.init.lhs, ast.IdentExp):
                        break
                err(
                    'orio.module.ortildriver.ast_util:%s: loop initialization expression not in "<id> = <exp>" form'
                    % stmt.init.line_no
                )
        ...
        if stmt.test:
            while True:
                ...
                if isinstance(stmt.test, ast.BinOpExp) and stmt.test.op_type in (
                    ast.BinOpExp.LT,
                    ast.BinOpExp.LE,
                ):
                    ...
                    if isinstance(stmt.test.lhs, ast.IdentExp):
                        break
                err(
                    'orio.module.ortildriver.ast_util:%s: loop test expression not in "<id> <= <exp>" or '
        ...
        if stmt.iter:
            while True:
                ...
                if (
                    isinstance(stmt.iter, ast.BinOpExp)
                    ...
                    if isinstance(stmt.iter.lhs, ast.IdentExp):
                        if isinstance(
                            ...
                            if (
                                isinstance(stmt.iter.rhs.lhs, ast.IdentExp)
                                and stmt.iter.lhs.name == stmt.iter.rhs.lhs.name
                            ):
                                break
                elif isinstance(stmt.iter, ast.UnaryExp) and stmt.iter.op_type in (
                    ast.UnaryExp.POST_INC,
                    ast.UnaryExp.PRE_INC,
                    ast.UnaryExp.POST_DEC,
                ...
                err(
                    (
                        'orio.module.ortildriver.ast_util:%s: loop iteration expression not in "<id>++" or "<id>--" or '
        ...
        if not stmt.init and not stmt.test and not stmt.iter:
            err(
                "orio.module.ortildriver.ast_util:%s: a loop with an empty control expression cannot be handled"
                % stmt.line_no
            )
        ...
        if stmt.iter:
            if isinstance(stmt.iter, ast.BinOpExp):
                ...
                assert isinstance(
                    stmt.iter, ast.UnaryExp
                ), "internal error:OrTilDriver: not unary"
        ...
        if inames.count(inames[0]) != len(inames):
            err(
                "orio.module.ortildriver.ast_util:%s: iterator names across init, test, and iter exps must be the same"
                % stmt.line_no
            )
        ...
        if stmt.test:
            if stmt.test.op_type == ast.BinOpExp.LT:
                ubound_exp = ast.BinOpExp(
                    stmt.test.rhs.replicate(),
        ...
        if stmt.iter:
            if isinstance(stmt.iter, ast.BinOpExp):
                ...
                if stmt.iter.op_type in (ast.UnaryExp.POST_INC, ast.UnaryExp.PRE_INC):
                    ...
                    err(
                        "orio.module.ortildriver.ast_util internal error: unexpected unary operation type"
                    )
            else:
                err(
                    "orio.module.ortildriver.ast_util internal error: unexpected type of iteration expression"
                )
    ...
    def createForLoop(self, index_id, lbound_exp, ubound_exp, stride_exp, loop_body):
        ...
        iter_exp = None
        if lbound_exp:
            init_exp = ast.BinOpExp(
                index_id.replicate(), lbound_exp.replicate(), ast.BinOpExp.EQ_ASGN
            )
        if ubound_exp:
            test_exp = ast.BinOpExp(
                index_id.replicate(), ubound_exp.replicate(), ast.BinOpExp.LE
            )
        if stride_exp:
            while isinstance(stride_exp, ast.ParenthExp):
                stride_exp = stride_exp.exp
            it = ast.BinOpExp(
                index_id.replicate(), stride_exp.replicate(), ast.BinOpExp.ADD
            )
