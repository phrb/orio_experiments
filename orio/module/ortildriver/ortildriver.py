...
class OrTilDriver(orio.module.module.Module):
    ...
    def __extractCoreTiles(self, code, tile_sizes):
        ...
        while True:
            ...
            if not m:
                err(
                    "orio.module.ortildriver.ortildriver: missing closing directive for full core tiles region"
                )
            ...
            if header_iters != trailer_iters:
                err(
                    "orio.module.ortildriver.ortildriver: different loop iterators in the opening and closing "
            ...
            for v in tile_size_vars:
                if v not in tile_size_table:
                    err(
                        'orio.module.ortildriver.ortildriver: undefined tile size variable: "%s"'
                        % v
                    )
