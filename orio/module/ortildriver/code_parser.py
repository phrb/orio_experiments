...
def t_error(t):
    err(
        'orio.module.ortildriver.code_parser: OrTilDriver:%s: syntactical error: "%s"'
        % (getLineNumber(t.lineno), t.value[0])
    )
...
def p_assignment_expression_2(p):
    ...
    if p[2] == "=":
        p[0] = ast.BinOpExp(
            p[1], p[3], ast.BinOpExp.EQ_ASGN, getLineNumber(p.lineno(1))
        )
    elif p[2] in ("*=", "/=", "%=", "+=", "-="):
        lhs = p[1].replicate()
        rhs = None
        if p[2] == "*=":
            ...
            err(
                "orio.module.ortildriver.code_parser internal error: unknown assignment operator"
            )
...
def p_equality_expression_2(p):
    ...
    if p[2] == "==":
        ...
        err(
            "orio.module.ortildriver.code_parser internal error: unknown equality operator"
        )
...
def p_relational_expression_2(p):
    ...
    if p[2] == "<":
        ...
        err(
            "orio.module.ortildriver.code_parser internal error: unknown relational operator"
        )
...
def p_additive_expression_2(p):
    ...
    if p[2] == "+":
        ...
        err(
            "orio.module.ortildriver.code_parser internal error: unknown additive operator"
        )
...
def p_multiplicative_expression_2(p):
    ...
    if p[2] == "*":
        ...
        err(
            "orio.module.ortildriver.code_parser internal error: unknown multiplicative operator"
        )
...
def p_unary_expression_4(p):
    ...
    if p[1] == "+":
        ...
        err(
            "orio.module.ortildriver.code_parser internal error: unknown unary operator"
        )
...
def p_error(p):
    err(
        'orio.module.ortildriver.code_parser: OrTilDriver:%s: grammatical error: "%s"'
        % (getLineNumber(p.lineno), p.value)
    )
