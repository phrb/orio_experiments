...
class ASTUtil:
    ...
    def getLoopIters(self, stmt):
        ...
        if isinstance(stmt, ast.ExpStmt):
            ...
            err(
                "orio.module.tilic.ast_util internal error: unknown type of statement: %s"
    ...
    def containIdentName(self, exp, iname):
        ...
        if not isinstance(exp, ast.Exp):
            err(
                "orio.module.tilic.ast_util internal error: input not an expression type"
            )
        ...
        if isinstance(exp, ast.NumLitExp):
            ...
            err(
                'orio.module.tilic.ast_util  internal error: unexpected AST type: "%s"'
    ...
    def getForLoopInfo(self, stmt):
        ...
        if stmt.init:
            if not (
                ...
                err(
                    'orio.module.tilic.ast_util: Tilic: loop initialization not in "id = lb" form'
                )

        # check loop test
        if stmt.test:
            if not (
                isinstance(stmt.test, ast.BinOpExp)
                and stmt.test.op_type in (ast.BinOpExp.LE, ast.BinOpExp.LT)
                and isinstance(stmt.test.lhs, ast.IdentExp)
            ):
                err(
                    'orio.module.tilic.ast_util: Tilic: loop test not in "id <= ub" or "id < ub" form'
                )
        ...
        if stmt.iter:
            if not (
                ...
                or (
                    isinstance(stmt.iter, ast.UnaryExp)
                    and isinstance(stmt.iter.exp, ast.IdentExp)
                    and stmt.iter.op_type
                    in (ast.UnaryExp.PRE_INC, ast.UnaryExp.POST_INC)
                )
            ):
                err(
                    'orio.module.tilic.ast_util: Tilic: loop iteration not in "id++" or "id += st" or "id = id + st" form'
                )

        # check if the control expressions are all empty
        if not stmt.init and not stmt.test and not stmt.iter:
            err(
                "orio.module.tilic.ast_util: Tilic: loop with an empty control expression cannot be handled"
            )
        ...
        if inames.count(inames[0]) != len(inames):
            err(
                "orio.module.tilic.ast_util: Tilic: different iterator names used in the loop control expressions"
            )
