    ...
    def pprint(self, tnode, indent="  ", extra_indent="  "):
        ...
        if isinstance(tnode, ast.NumLitExp):
            ...
            if tnode.op_type == tnode.PLUS:
                ...
                err(
                    "orio.module.tilic.pprinter internal error: unknown unary operator type: %s"
                    % tnode.op_type
                )
            ...
            if tnode.op_type == tnode.MUL:
                ...
                err(
                    "orio.module.tilic.pprinter internal error: unknown binary operator type: %s"
                    % tnode.op_type
                )
            ...
            if tnode.iter:
                if (
                    isinstance(tnode.iter, ast.BinOpExp)
                    and tnode.iter.op_type == ast.BinOpExp.EQ_ASGN
                    and isinstance(tnode.iter.rhs, ast.BinOpExp)
                    and tnode.iter.rhs.op_type == ast.BinOpExp.ADD
                    and isinstance(tnode.iter.rhs.lhs, ast.IdentExp)
                    and isinstance(tnode.iter.lhs, ast.IdentExp)
                    and tnode.iter.lhs.name == tnode.iter.rhs.lhs.name
                ):
                    s += (
                        str(tnode.iter.lhs.name)
                        + "+="
                        + self.pprint(tnode.iter.rhs.rhs, indent, extra_indent)
                    )
                else:
            ...
            err(
                "orio.module.tilic.pprinter internal error: unrecognized type of AST: %s"
