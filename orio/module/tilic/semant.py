    ...
    def __normalizeStmt(self, stmt):
        ...
        * To remove all meaningless scopings inside each compound statement.
        ...
        if isinstance(stmt, ast.ExpStmt):
            ...
            err(
                "orio.module.tilic.semant internal error: unknown type of statement: %s"
        ...
        if isinstance(stmt, ast.ExpStmt):
            ...
            for s in stmt.stmts:
                if isinstance(s, ast.CompStmt):
                    err(
                        "orio.module.tilic.semant: Tilic: a compound statement cannot be directly nested in another compound statement"
                    )
            ...
            if id.name in oloop_inames:
                err(
                    "orio.module.tilic.semant: Tilic: illegal loop nest where an inner loop has the same iterator name as the outer loop"
                )
