    ...
    def transform(self):
        ...
        for t in tile_sizes:
            if not isinstance(t, int) or t <= 0:
                err(
                    "orio.module.pluto.pluto: Pluto: tile size must be a positive integer"
                )
        if not isinstance(tile_level, int) or not (0 <= tile_level <= 2):
            err(
                "orio.module.pluto.pluto: Pluto: number of tile levels must be either 0, 1, or 2"
            )
        if not isinstance(unroll_factor, int) or unroll_factor < 1:
            err(
                "orio.module.pluto.pluto: Pluto: invalid unroll factor: %s"
                % unroll_factor
            )
        ...
        if (not open_m) or (not close_m):
            err(
                "orio.module.pluto.pluto: cannot find the opening and closing tags for the Pluto code"
            )

        # check if Pluto has been correctly installed
        if os.popen("polycc").read() == "":
            err(
                'orio.module.pluto.pluto:  Pluto is not installed. Cannot use "polycc" command.'
            )
