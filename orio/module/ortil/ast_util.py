...
class ASTUtil:
    ...
    def getLoopIters(self, stmt):
        ...
        if isinstance(stmt, ast.ExpStmt):
            ...
            err(
                "orio.module.ortil.ast_util internal error:OrTil: unknown type of statement: %s"
    ...
    def containIdentName(self, exp, iname):
        ...
        if not isinstance(exp, ast.Exp):
            err(
                "orio.module.ortil.ast_util internal error:OrTil:containIdentName: input not an expression type"
            )
        ...
        if isinstance(exp, ast.NumLitExp):
            ...
            err(
                'orio.module.ortil.ast_util internal error:OrTil: unexpected AST type: "%s"'
    ...
    def getForLoopInfo(self, stmt):
        ...
        if not isinstance(stmt, ast.ForStmt):
            err(
                "orio.module.ortil.ast_util: OrTil:%s: not a for-loop statement"
                % stmt.line_no
            )
            ...
            while True:
                ...
                if (
                    ...
                    if isinstance(stmt.init.lhs, ast.IdentExp):
                        break
                err(
                    'orio.module.ortil.ast_util:%s: loop initialization expression not in "<id> = <exp>" form'
                    % stmt.init.line_no
                )
            ...
            while True:
                ...
                if isinstance(stmt.test, ast.BinOpExp) and stmt.test.op_type in (
                    ...
                    if isinstance(stmt.test.lhs, ast.IdentExp):
                        break
                err(
                    'orio.module.ortil.ast_util:%s: loop test expression not in "<id> <= <exp>" or '
        ...
        if not stmt.init and not stmt.test and not stmt.iter:
            err(
                "orio.module.ortil.ast_util:%s: a loop with an empty control expression cannot be handled"
                % stmt.line_no
            )
        ...
        if stmt.iter:
            if isinstance(stmt.iter, ast.BinOpExp):
                ...
                ), "internal error:OrTil: not unary"
        ...
        if inames.count(inames[0]) != len(inames):
            err(
                "orio.module.ortil.ast_util:%s: iterator names across init, test, and iter exps must be the same"
                % stmt.line_no
            )
        ...
        if stmt.iter:
            if isinstance(stmt.iter, ast.BinOpExp):
                ...
                if stmt.iter.op_type in (ast.UnaryExp.POST_INC, ast.UnaryExp.PRE_INC):
                    ...
                    err(
                        "orio.module.ortil.ast_util internal error:OrTil: unexpected unary operation type"
                    )
            else:
                err(
                    "orio.module.ortil.ast_utilinternal error:OrTil: unexpected type of iteration expression"
                )
