...
class Transformation:
    ...
    def __init__(self, tiling_info):
        ...
        self.recursive_tile_level = (
            self.num_tile_level
        )  # the highest level of tiling used to
    ...
    def __getInterTileLoop(self, iter_name, tsize_name, lb_exp, ub_exp, st_exp, lbody):
        ...
        lb = lb_exp
        tmp = ast.BinOpExp(
            ast.IdentExp(tsize_name), ast.ParenthExp(st_exp), ast.BinOpExp.SUB
        )
    ...
    def __findMinMaxVal(self, min_or_max, exp, var_names, val_table, up_sign=1):
        ...
        # string expression
            ...
            err(
                "orio.module.ortil.transformation: OrTil: invalid string expression found in loop bound expression: %s"
                % exp
            )
        ...
        # identifier expression
            ...
            if min_or_max == "max":
                ...
                err(
                    "orio.module.ortil.transformation internal error: unrecognized min/max argument value"
                )
        ...
        # array reference expression
            ...
            err(
                "orio.module.ortil.transformation: invalid array-reference expression found in loop bound "
                + "expression: %s" % exp
            )
        ...
        # function call expression
            ...
            for a in exp.args:
                exp.args.append(
                    self.__findMinMaxVal(min_or_max, a, var_names, val_table, up_sign)
                )
        ...
        # unary operation expression
            ...
            if exp.op_type == ast.UnaryExp.MINUS:
                up_sign *= -1
            exp.exp = self.__findMinMaxVal(
                min_or_max, exp.exp, var_names, val_table, up_sign
            )
        ...
        # binary operation expression
            ...
            exp.lhs = self.__findMinMaxVal(
                min_or_max, exp.lhs, var_names, val_table, up_sign
            )
            if exp.op_type == ast.BinOpExp.SUB:
                up_sign *= -1
            exp.rhs = self.__findMinMaxVal(
                min_or_max, exp.rhs, var_names, val_table, up_sign
            )
        ...
        # unrecognized expression
            ...
            err(
                "orio.module.ortil.transformation internal error: unknown type of expression: %s"
    ...
    def __convertToASTs(
        self, dstmts, tile_level, contain_loop, loop_inames, loop_info_table, int_vars
    ):
        """
        ...
        while i < len(dstmts):
            ...
            if isinstance(s, tuple):
                ...
                # need to enclose with a multilevel inter-tile loop nest
                    ...
                    l = self.__getMultiLevelTileLoop(
                        tile_level, loop_inames, st_exps, lbody
                    )
                    asts.append(l)
            ...
            # if it's an if-statement's test expression
                ...
                if not isinstance(s, ast.BinOpExp):
                    err(
                        "orio.module.ortil.transformation internal error: a test expression is expected"
                    )

                # generate AST for the true statement
                t1 = self.__convertToASTs(
                    dstmts[i + 1],
                    tile_level,
                    contain_loop,
                    loop_inames,
    ...
    def __tile(
        self, stmt, tile_level, outer_loop_infos, preceding_stmts, lbound_info, int_vars
    ):
        ...
        if not isinstance(tile_level, int) or tile_level <= 0:
            err(
                "orio.module.ortil.transformation internal error: invalid tiling level: %s"
                % tile_level
            )
        ...
        if isinstance(stmt, ast.CompStmt):
            err(
                "orio.module.ortil.transformation internal error: unexpected compound statement directly nested inside "
        ...
        if isinstance(stmt, ast.ForStmt):
            ...
            if lbound_info:
                lb_name, ub_name, need_prolog, need_epilog, need_tiled_loop = (
                    lbound_info
                )
                ...
                if not need_tiled_loop:
                    err(
                        "orio.module.ortil.transformation internal error: unexpected case where generation of the orio.main."
                        + "rectangular tiled loop is needed"
                    )

            # get explicit loop-bound scanning code
            t = self.__getLoopBoundScanningStmts(
                this_lbody.stmts, tile_level, n_outer_loop_inames, n_loop_info_table
            )
            ...
            if need_prolog:
                ub = ast.BinOpExp(
                    rect_lb_exp, ast.ParenthExp(this_st_exp), ast.BinOpExp.SUB
                )
            ...
            for s, binfo in zip(this_lbody.stmts, lbound_info_seq):
                ...
                for p_stmts in if_branches:
                    ...
                    if binfo == None:
                        n_p_stmts = self.__tile(
                            s, tile_level, n_outer_loop_infos, p_stmts, binfo, int_vars
                        )
                        while len(p_stmts) > 0:
                            p_stmts.pop()
                    ...
                    lbn, ubn, _, _, _ = binfo
                    test_exp = ast.BinOpExp(
                        ast.IdentExp(lbn), ast.IdentExp(ubn), ast.BinOpExp.LT
                    )
            ...
            tiled_code = self.__getInterTileLoop(
                iname, tname, rect_lb_exp, rect_ub_exp, this_st_exp, lbody
            )
